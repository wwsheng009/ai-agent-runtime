...
- "Where should I create this skill? If you do not have a preference, I will place it in `./.agents/skills` for this workspace so Codex can discover it automatically; if you want a personal skill instead, I will use `~/.aicli/skills`, or `~/.aicli/agents/skills` if you want the alternate user-root layout."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to `./.agents/skills` when working in this repository so the runtime auto-loads it; when the user wants a personal skill instead, use `~/.aicli/skills` and only switch to `~/.aicli/agents/skills` if they explicitly ask for that alternate layout.
