#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into a destination skills directory."""
...
def _repo_root() -> str | None:
    ...
    for parent in script_path.parents:
        if (parent / "backend" / "configs" / "config.yaml").is_file():
            return str(parent)
    return None
...
def _default_dest() -> str:
    ...
    return os.path.join(str(Path.home()), ".aicli", "skills")


def _parse_args(argv: list[str]) -> Args:
    parser = argparse.ArgumentParser(description="Install a skill from GitHub into a destination skills directory.")
    ...
    parser.epilog = (
        "When run from this repository's skill package, the default destination "
        "is the workspace-local ./.agents/skills directory; otherwise it falls "
        "back to ~/.aicli/skills. Use --dest ~/.aicli/agents/skills if you need "
        "the alternate user skills root."
    )
