...
// 注意：provider / model / reasoning_effort 在 Web 客户端由底部 composer 面板的 cfg-bar 实时
// 展示，此处不重复。
//
// 响应示例：
//
...
func HandleChatWebAPIStatusLine(w http.ResponseWriter, r *http.Request) {
	...
	// 状态栏含每秒推进的时钟段，ETag 无意义（每次内容都变）；只做 gzip 协商。
	writeWebAPIJSONBody(w, r, http.StatusOK, snap, "")
}
...
func buildChatWebStatusBarSnapshotForSession(session *ChatSession) *chatWebStatusBarSnapshot {
	...
	// provider  / model / reasoning_effort 已在底部 composer 面板的 cfg-bar 实时展示，
