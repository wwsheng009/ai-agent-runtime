...
// SynchronizedUpdateBeginSequence / SynchronizedUpdateEndSequence frame a batch
// of terminal writes with DEC private mode 2026 so the emulator applies the
// whole repaint atomically (no half-drawn frame / tearing). Terminals that do
// not implement 2026 ignore the unknown private mode, so emitting it is always
// safe.
const (
