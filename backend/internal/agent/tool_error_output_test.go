...
func TestExecuteToolCall_PreservesRawOutputOnToolError(t *testing.T) {
	...
	assert.Contains(t, message.Content, "Tool execution failed: exit status 1\nstderr line 1\nstderr line 2")
	assert.Contains(t, message.Content, "Full raw output artifact_id: art_")
	...
	assert.Equal(t, "TOOL_EXECUTION", completed[0].Payload["error_code"])
