...
// configFileOSLockSupported 标记本平台没有跨进程锁实现（测试据此跳过跨进程用例）。
const configFileOSLockSupported = false

// acquireConfigFileOSLock 是跨进程锁的平台兜底实现（no-op）。
//
...
func acquireConfigFileOSLock(string) (func(), bool) {
	return nil, false
}
