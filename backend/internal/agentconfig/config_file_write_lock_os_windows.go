...
const (
	configFileOSLockBytesLow  = 0xFFFFFFFF
	configFileOSLockBytesHigh = 0xFFFFFFFF
)
...
// acquireConfigFileOSLock 在 Windows 上用 LockFileEx 取跨进程排他锁（整文件范围）。
//
...
// 删除会与另一个进程的「打开 → 加锁」形成竞态（见 config_file_write_lock.go）。
//
...
func acquireConfigFileOSLock(lockBasePath string) (func(), bool) {
	...
	if err := windows.LockFileEx(
		...
		0,
		configFileOSLockBytesLow,
		configFileOSLockBytesHigh,
		overlapped,
	); err != nil {
	...
	return func() {
		once.Do(func() {
			...
			_ = windows.UnlockFileEx(handle, 0, configFileOSLockBytesLow, configFileOSLockBytesHigh, overlapped)
			_ = windows.CloseHandle(handle)
