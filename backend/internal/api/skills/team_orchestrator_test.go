...
func TestSessionActorClientTriggerTaskPersistsDispatchEvents(t *testing.T) {
	...
	assert.Equal(t, team.TaskDispatchStartedEvent, events[1].Type)
	assert.Equal(t, team.TaskDispatchCompletedEvent, events[2].Type)
	assert.Equal(t, "mate-1", events[2].Payload["assignee"])
	assert.Equal(t, "session-1", events[2].Payload["session_id"])
	assert.Equal(t, false, events[2].Payload["success"])
	assert.Contains(t, events[2].Payload["error"], "session hub not configured")
