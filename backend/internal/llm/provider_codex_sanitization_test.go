...
func TestProviderWrapperConvertRequest_SanitizesCodexOrphanToolReplayMessages(t *testing.T) {
	...
	if got := converted.Metadata["tool_replay_sanitized"]; got != true {
		t.Fatalf("expected tool replay sanitization metadata, got %#v", converted.Metadata)
	}
	if got := converted.Metadata["tool_replay_dropped_messages"]; got != 1 {
		t.Fatalf("expected one dropped orphan tool message, got %#v", converted.Metadata)
	}
}

func TestProviderWrapperConvertRequest_SanitizesOpenAIOrphanToolReplayMessages(t *testing.T) {
	provider := &ProviderWrapper{
		config: &ProviderConfig{
			Type: "openai",
		},
