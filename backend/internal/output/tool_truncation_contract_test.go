...
func TestToolTruncatedUpstreamVocabulary(t *testing.T) {
	cases := []struct {
		...
		// Capture-layer facts are not tool-owned truncation: an exec capture
		// that hit its retention limit says nothing about the model-visible
		// window, so the render layer must still fold (and size) the payload.
		{"output_truncated/true", map[string]interface{}{"output_truncated": true}, false},
...
// TestToolFoldedOwnWindowVocabulary pins the narrower "bytes are missing from
// the body" gate that decides whether a tool-owned payload still carries the
// raw-output pointer: only real fold metadata qualifies, capture-level flags do
// not.
func TestToolFoldedOwnWindowVocabulary(t *testing.T) {
	cases := []struct {
		name string
		meta map[string]interface{}
		want bool
		...
		{"nil", nil, false},
		{"intact-view", map[string]interface{}{"is_truncated": false, "skip_render_truncation": true}, false},
		{"folded-view", map[string]interface{}{"is_truncated": true, "skip_render_truncation": true}, true},
		{"folded-grep", map[string]interface{}{"results_truncated": true}, true},
		{"folded-fetch", map[string]interface{}{"truncated": true}, true},
		{"capture-only", map[string]interface{}{"output_truncated": true}, false},
		{"skip-only", map[string]interface{}{"skip_render_truncation": true}, false},
		{
			"nested-tool-metadata",
			map[string]interface{}{
				"tool_metadata": map[string]interface{}{"is_truncated": true},
			},
			true,
		},
	}
	for _, tc := range cases {
		if got := toolFoldedOwnWindow(tc.meta); got != tc.want {
			t.Errorf("%s: toolFoldedOwnWindow=%v want %v", tc.name, got, tc.want)
		}
	}
}
