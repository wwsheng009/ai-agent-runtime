...
func RenderMultiAgentCollaborationGuidance() string {
	lines := []string{
		...
		"- Business supervision flow: spawn (wait or background) -> inspect with subagent_status, one call returning the whole N-row ledger (pending_count/terminal_count/terminal_delta plus per-row action_required/allowed_actions; add include_digest=true to fold the lifecycle digest with the notification rows), instead of polling wait_agent repeatedly (subagent_inspect_task is the byte-bounded deep look at one subject; such inspections are exempt from the anti-polling advisory) -> report the parent-turn digest (critical_unresolved/action_required rows plus the batch progress block) -> converge finished children with close_agent once the batch reaches terminal (use subagent_control close only while the row is still unresolved, since a closed row accepts inspect alone).",
	}
	return strings.Join(lines, "\n")
}
