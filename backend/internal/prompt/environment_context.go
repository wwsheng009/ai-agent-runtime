...
func RenderMultiAgentCollaborationGuidance() string {
	lines := []string{
		...
		"- Business supervision flow: spawn (wait or background) -> inspect with supervision_descendants, one call returning the whole N-row state matrix, instead of polling wait_agent repeatedly (such inspections are exempt from the anti-polling advisory) -> report the parent-turn digest (critical_unresolved/action_required rows plus the batch progress block) -> converge finished children with close_agent once the batch reaches terminal (use control_descendant close only while the row is still unresolved, since a closed row accepts inspect alone).",
	}
	return strings.Join(lines, "\n")
}
