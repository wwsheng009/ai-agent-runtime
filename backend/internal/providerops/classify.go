...
// ModelGroup 是 fetch-models 响应中的一个协议分组：按 model
...
// Classification 是一次 fetch-models 的分类结果。
...
// ---------------------------------------------------------------------------
// 导出面：commands 侧 provider_fetch_models_classify.go 的薄转发层目标。
// ---------------------------------------------------------------------------

// ClassifyFetchedModels 对 /models 拉取结果做协议 / provider template 分组，
// 并选出与当前 provider 协议一致的主组（login 分类链路的复用入口）。
func ClassifyFetchedModels(
	providerName string,
	...
	return classifyFetchedModels(providerName, provider, requestedLoginProtocol, groupingLoginProtocol, models, cfg)
}

// SplitProviderFetchPrimaryModels 把主组模型拆成 verified（有非 fallback 卡
// 数据支持）/ assumed（仅协议 fallback 假定）两部分。
func SplitProviderFetchPrimaryModels(
	catalog *modelcard.Catalog,
	providerName, loginProtocol string,
	provider config.Provider,
	group LoginModelGroup,
) (verified, assumed []ModelInfo) {
	return splitProviderFetchPrimaryModels(catalog, providerName, loginProtocol, provider, group)
}

// ProviderFetchModelHasCardBacking 判定模型在主组 template 上是否有非
// fallback 卡片命中；组无 template（协议兜底组）时一律 false。
func ProviderFetchModelHasCardBacking(catalog *modelcard.Catalog, ctx modelcard.Context, modelID string, group LoginModelGroup) bool {
	return providerFetchModelHasCardBacking(catalog, ctx, modelID, group)
}

// ProviderFetchGroupModelsOutsidePrimary 返回组内不属于主组的模型 ID，
// 用于跨组去重后的“被过滤”计数。
func ProviderFetchGroupModelsOutsidePrimary(models []ModelInfo, primaryIDs map[string]struct{}) []string {
	return providerFetchGroupModelsOutsidePrimary(models, primaryIDs)
}

// MatchFetchedModelMetadata 为 fetch-models 拉取到的模型重新匹配 reasoning
// 元数据（覆盖语义：已保存 capabilities 不参与合并），返回 model id →
// 前端 reasoning 编辑器可直接回显的配置视图。
func MatchFetchedModelMetadata(
	providerName, loginProtocol string,
	provider config.Provider,
	models []ModelInfo,
	cfg *config.Config,
) map[string]ChatWebConfigModel {
	return matchFetchedModelMetadata(providerName, loginProtocol, provider, models, cfg)
}

// classifyFetchedModels 复用 aicli login 的模型分类链路
// （NormalizeModelsForProtocol →
...
func classifyFetchedModels(
	...
	catalog, _, err := loadModelCardCatalog(CatalogOptions{}, cfg)
	if err != nil || catalog == nil {
		return nil
	}
	normalized := NormalizeModelsForProtocol(models, groupingLoginProtocol)
	if len(normalized) == 0 {
		return nil
	}
	...
	for _, id := range ModelIDs(primary.Models) {
...
func splitProviderFetchPrimaryModels(
	...
	currentTemplate, _ := ResolveProviderTemplate(catalog, runtimeProtocol, provider)
...
func providerFetchGroupModelsOutsidePrimary(models []ModelInfo, primaryIDs map[string]struct{}) []string {
	...
	for _, id := range ModelIDs(models) {
...
// ResolveFetchModelsLoginProtocol 归一化 fetch-models 请求中的协议：
// 表单协议为空 / auto 时回退到 provider 已保存协议；仍不可用则按 openai
// 处理（与 fetchModels 对空协议的 models path 解析默认一致）。
func ResolveFetchModelsLoginProtocol(requested string, provider config.Provider) string {
...
// matchFetchedModelMetadata 为 fetch-models 本次拉取到的模型重新匹配
// reasoning 相关元数据（/models 端点自带元数据 → model card → 协议兼容默认值，
// 与 aicli login 的 buildProviderLoginModelCapabilitiesForLogin 同源），返回
// model id → 前端 reasoning 编辑器可直接回显的字段视图。
//
// 与 login 的关键差异：这里刻意传入空的 existing capabilities——「获取模型
// 列表」是“覆盖重匹配”语义，已保存的旧 reasoning 配置不参与合并，否则旧值会
// 作为最高优先级遮蔽元数据匹配结果（用户执行获取后仍看到过期配置）。
// 元数据匹配为空的模型不进入返回值，由前端按“无元数据 → 清空旧配置”处理。
func matchFetchedModelMetadata(
	providerName, loginProtocol string,
	provider config.Provider,
	models []ModelInfo,
	...
	matched := MatchMetadata(MetadataRequest{
		ProviderName:  providerName,
		LoginProtocol: loginProtocol,
		Provider:      provider,
		Models:        models,
