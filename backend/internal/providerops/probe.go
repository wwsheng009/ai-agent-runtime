...
// ModelSingleProbe 是一次模型 × 协议探测的结果（CLI/web client 与契约
// ProbeModels 共用）。
type ModelSingleProbe struct {
...
func probeProviderModelOnce(client *http.Client, provider config.Provider, probeProtocol, modelID string, timeout time.Duration) ModelSingleProbe {
	probe := ModelSingleProbe{Protocol: RuntimeProtocolForLoginProtocol(probeProtocol)}
...
// ProbeMatrixRequest 是一次批量探测的输入。
type ProbeMatrixRequest struct {
...
// ProbeMatrix 并发执行模型 × 协议探测矩阵。结果按输入顺序排列
// （按索引写回，无需锁）；默认并发 6、单请求超时 15s，防止探测拖垮上游。
func ProbeMatrix(req ProbeMatrixRequest) []ModelProbeMatrix {
	...
	results := make([]ModelProbeMatrix, len(models))
	...
	for i, model := range models {
		results[i] = ModelProbeMatrix{
			Model:  model,
			Probes: make([]ModelSingleProbe, len(protocols)),
		}
...
// runModelProbes 是包内既有点名的内部包装。
func runModelProbes(req ProbeMatrixRequest) []ModelProbeMatrix {
	return ProbeMatrix(req)
}
...
// ---------------------------------------------------------------------------
// 导出面：CLI（aicli provider model-probe / web client）与 runtime server
// （/api/runtime/providers/probe-models）共用同一份探测实现，以下导出别名
// 是 commands 侧 provider_model_probe.go 薄转发层的目标。
// ---------------------------------------------------------------------------

// ProbeVerdict 是单个模型 × 协议探测结论的类型化词表。
type ProbeVerdict = providerModelProbeVerdict

const (
	ProbeVerdictSupported   = probeVerdictSupported   // 实测支持
	ProbeVerdictUnsupported = probeVerdictUnsupported // 网关明确拒绝
	ProbeVerdictUnknown     = probeVerdictUnknown     // 无法定性（鉴权/限流/区域等）
)

// ProviderProbeSessionID 生成探测请求专用的合成会话 ID（每次探测运行唯一，
// 请求头模板里的 {session_id} 占位符用它替换）。
func ProviderProbeSessionID() string { return providerProbeSessionID() }

// ClassifyProviderModelProbeMessage 按错误措辞给探测结论；无法识别的措辞
// 一律 unknown——负向结论只认明确拒绝该模型/格式的网关措辞。
func ClassifyProviderModelProbeMessage(message string) ProbeVerdict {
	return classifyProviderModelProbeMessage(message)
}

// ClassifyProviderModelProbeResponse 按 HTTP 状态码 + 响应体结构给探测结论。
func ClassifyProviderModelProbeResponse(statusCode int, body []byte, protocol string) (ProbeVerdict, string) {
	return classifyProviderModelProbeResponse(statusCode, body, protocol)
}

// ProbeResponseLooksSuccessful 判断 200 响应体是否具备对应协议可识别的补全结构。
func ProbeResponseLooksSuccessful(protocol string, payload map[string]interface{}) bool {
	return probeResponseLooksSuccessful(protocol, payload)
}

// ProviderProbeBaseURLHost 提取 base_url 的 host（小写），用于把探测卡片
// 限定在该网关范围内。
func ProviderProbeBaseURLHost(baseURL string) string { return providerProbeBaseURLHost(baseURL) }

// ProviderProbeCardID 生成探测卡片 ID：probe.<provider|host>.<model>.<protocol>。
func ProviderProbeCardID(providerName, host, modelID, protocol string) string {
	return providerProbeCardID(providerName, host, modelID, protocol)
}

// AppendProviderModelProbeCards 把 supported 探测结论写回用户级
// model_cards.yaml（unknown/unsupported 不落卡，手工卡优先，先备份再重写）。
func AppendProviderModelProbeCards(
	cfg *config.Config,
	providerName string,
	provider config.Provider,
	...
	return appendProviderModelProbeCards(cfg, providerName, provider, results)
}
