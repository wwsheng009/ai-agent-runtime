...
func NewRuntimeConfigLayersProvider() runtimeapi.RuntimeConfigLayersProvider {
	return func() []runtimeapi.ConfigDocumentLayer {
		layers := config.RuntimeConfigLayerStack()
		out := make([]runtimeapi.ConfigDocumentLayer, 0, len(layers))
		for _, layer := range layers {
			...
			out = append(out, runtimeapi.ConfigDocumentLayer{
