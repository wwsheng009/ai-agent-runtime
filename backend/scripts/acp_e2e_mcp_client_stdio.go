// Real end-to-end ACP verification of the P1 positive path: a stdio MCP server
// supplied by the *client* through session/new is connected, its tool is in the
// tool surface of the FIRST prompt and callable, and its child process is reaped
// by session/close (docs/plan/acp-mcp-integration-implementation-plan-20260920.md
// §6.2 acceptance criteria 1 + 4).
//
// Trust setup: folder trust is enabled (AICLI_FOLDER_TRUST=1) and the isolated
// HOME carries an explicit trust record for the temp workspace, so the client
// server passes the gate. The local config chain contributes nothing here
// (no ~/.aicli/mcp.yaml), which isolates the client-supplied path.
//
// Run (from backend/):
//
//	go run ./scripts/acp_e2e_mcp_client_stdio.go [aicli.exe]
//
...
func run() error {
	...
	tmpRoot, err := os.MkdirTemp("", "acp-e2e-mcp-client")
	if err != nil {
		return err
	}
	...
	// explicit trust record for this workspace, so the client server passes the
	// gate without touching production defaults.
	...
	captured := &capture{}
	srv := httptest.NewServer(&mockProvider{captured: captured, sentinel: probeSentinel, tool: clientTool})
	defer srv.Close()
...
`, yamlQuote(agentLog), srv.URL)
	...
	for attempt := 1; attempt <= maxPromptAttempts; attempt++ {
		...
		for _, name := range lastTools {
			if matchesToolName(name, clientTool) {
				visible = true
				break
			}
		}
		...
		fmt.Printf("OK prompt #%d upstream_tools=%d client_tool_visible=%v callable=%v\n",
	...
	if firstVisible == 0 {
		return fmt.Errorf("client-supplied MCP tool %q never appeared in the model tool surface within %d prompts\nHINT: the server IS started, connected and catalog-registered (see the acp.mcp / \"registered N MCP tool(s) after connect\" lines below), but a client-supplied stdio server always finishes its handshake after session bootstrap. The late tool must be re-admitted at the next turn boundary on BOTH paths: (1) the surface-derived tool policy allowlist (snapshotted when the agent is built - cmd/aicli/commands/chat_actor_host.go buildLocalChatToolPolicy) must be re-synced by syncLocalChatToolPolicyAllowlist (cmd/aicli/commands/chat_tool_policy_sync.go, wired into localChatPrepareRunHook); (2) the session stable tool surface must be invalidated/rebuilt (cmd/aicli/commands/chat_mcp_surface_invalidation.go + internal/agent loop.shouldRefreshStableToolSurface). Check the agent log for the \"tool policy allowlist synced with live tool surface\" line.\nlast upstream tools=%v\nagent stderr:\n%s\nagent log (mcp/tool lines):\n%s",
			clientTool, maxPromptAttempts, lastTools, a.stderr.dump(25), logDigest(agentLog, 40))
	}
	if firstVisible != 1 {
		return fmt.Errorf("client-supplied MCP tool %q was NOT in the FIRST prompt tool surface (first visible at prompt #%d) - §6.2 acceptance criterion 1\nagent log (mcp/tool lines):\n%s",
			clientTool, firstVisible, logDigest(agentLog, 40))
	}
	if !callable {
		return fmt.Errorf("client-supplied MCP tool %q was visible at prompt #1 but its echo result never reached the provider (tool not executable)\nagent stderr:\n%s",
			clientTool, a.stderr.dump(25))
	}
	fmt.Printf("OK client-supplied MCP tool usable at the FIRST prompt: tool=%s server=%s\n", clientTool, clientServerName)

	// session/close must reap the stdio child of the client-supplied server.
	aliveBefore := processAlive(pid)
	fmt.Printf("DIAG before session/close: client-server-alive=%v\n", aliveBefore)
	if !aliveBefore {
		return fmt.Errorf("client-supplied stdio server pid=%d is not alive before session/close; reaping cannot be proven", pid)
	}
	if _, err := a.call("session/close", map[string]interface{}{"sessionId": snew.SessionID}, 60*time.Second); err != nil {
		return fmt.Errorf("session/close: %w\nagent stderr:\n%s", err, a.stderr.dump(25))
	}
	fmt.Println("OK session/close returned")
	...
	if processAlive(pid) {
		return fmt.Errorf("client-supplied stdio server pid=%d is STILL alive 20s after session/close (child not reaped)\nagent log (mcp/tool lines):\n%s",
			pid, logDigest(agentLog, 40))
	}
	fmt.Printf("OK client-supplied stdio server pid=%d reaped after session/close (tasklist/proc probe)\n", pid)
	if notes := toolCallNotifications(a.updatesSnapshot()); len(notes) > 0 {
		fmt.Printf("OK ACP tool_call notifications: %s\n", strings.Join(notes, ", "))
	}
	fmt.Println("E2E PASS acp_e2e_mcp_client_stdio")
	return nil
}

// clientServerName/clientTool describe the server that the ACP client sends in
// session/new; the tool name is unique so no built-in can shadow it.
const (
	clientServerName = "client-e2e"
	clientTool       = "acp_e2e_client_echo"
)
