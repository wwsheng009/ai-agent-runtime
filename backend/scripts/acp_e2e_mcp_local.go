// Real end-to-end ACP verification of the *local* MCP config chain (P0 baseline).
//
// What it proves (docs/plan/acp-mcp-integration-implementation-plan-20260920.md
// §6.1 deliverable 1 + §7.2 row 1): a stdio MCP server that lives in the local
// config chain (~/.aicli/mcp.yaml under an isolated USERPROFILE/HOME) is really
// connected by `aicli agent stdio` and its tool is both visible in the model
// tool surface and callable inside an ACP session.
//
// It also measures §7.4 item 1 / G6: whether a tool that arrives through the
// *asynchronous* MCP connect (StartAsync) is already visible at the FIRST
// prompt, or only at a later turn boundary. The measured answer is printed and
// the script fails if the tool never becomes usable within maxPromptAttempts.
//
// Flow:
//  1. build the aicli binary (unless one is passed as argv[1]) and the stdio MCP
//     helper server (scripts/acp_e2e_mcp_stdio_helper.go)
//  2. isolated HOME with config.yaml (mock provider) + mcp.yaml (helper server)
//  3. initialize + session/new (no client-provided mcpServers) + session/prompt
//  4. assert the helper tool is in the upstream tool surface and that its
//     "echo:<text>" result is fed back to the model
//
// Run (from backend/):
//
//	go run ./scripts/acp_e2e_mcp_local.go [aicli.exe]
//
...
func run() error {
	...
	tmpRoot, err := os.MkdirTemp("", "acp-e2e-mcp-local")
	if err != nil {
		return err
	}
	...
	markerPath := filepath.Join(tmpRoot, "helper-started.marker")
	pidPath := filepath.Join(tmpRoot, "helper.pid")
...
`, yamlQuote(agentLog), srv.URL)
	...
	mcpYAML := fmt.Sprintf(`mcpServers:
  %s:
    name: %s
    type: stdio
    command: %s
    args: ['-tool', %s, '-marker', %s, '-pidfile', %s]
    enabled: true
    trustLevel: local
`, serverName, serverName, yamlQuote(helper), yamlQuote(toolName), yamlQuote(markerPath), yamlQuote(pidPath))
	mcpPath := filepath.Join(home, ".aicli", "mcp.yaml")
	if err := writeFile(mcpPath, mcpYAML); err != nil {
		return err
	}
	fmt.Printf("OK wrote local MCP config %s (server=%s tool=%s)\n", mcpPath, serverName, toolName)
	...
	fmt.Printf("DIAG after session/new: helper-marker=%s helper-pid=%s\n",
		fileState(markerPath), fileState(pidPath))
	...
	for attempt := 1; attempt <= maxPromptAttempts; attempt++ {
		...
		for _, name := range lastTools {
			if matchesToolName(name, toolName) {
				visible = true
				break
			}
		}
		...
		fmt.Printf("OK prompt #%d upstream_tools=%d mcp_tool_visible=%v callable=%v\n",
			attempt, len(lastTools), visible, callable)
		fmt.Printf("DIAG after prompt #%d: helper-marker=%s helper-pid=%s\n",
			attempt, fileState(markerPath), fileState(pidPath))
		if visible && callable {
			break
		}
	}
	...
	if firstVisible == 0 {
		return fmt.Errorf("MCP tool %q never appeared in the model tool surface within %d prompts\nHINT: a stdio server always finishes its handshake after session bootstrap, so the late tool has to be re-admitted at the next turn boundary on BOTH paths: (1) the surface-derived tool policy allowlist (snapshotted when the agent is built - cmd/aicli/commands/chat_actor_host.go buildLocalChatToolPolicy) must be re-synced by syncLocalChatToolPolicyAllowlist (cmd/aicli/commands/chat_tool_policy_sync.go, wired into localChatPrepareRunHook), otherwise AllowToolInfo rejects the tool and neither the model surface nor the refresh gate ever sees it; (2) the session stable tool surface must be invalidated/rebuilt (cmd/aicli/commands/chat_mcp_surface_invalidation.go + internal/agent loop.shouldRefreshStableToolSurface). Check the agent log for the \"registered N MCP tool(s) after connect\" line and the \"tool policy allowlist synced with live tool surface\" line.\nlast upstream tools=%v\nhelper-marker=%s helper-pid=%s\nagent stderr:\n%s\nagent log (mcp/tool lines):\n%s",
			toolName, maxPromptAttempts, lastTools, fileState(markerPath), fileState(pidPath), a.stderr.dump(25), logDigest(agentLog, 40))
	}
	if !callable {
		return fmt.Errorf("MCP tool %q was visible at prompt #%d but its echo result never reached the provider (tool not executable)\nagent stderr:\n%s",
			toolName, firstVisible, a.stderr.dump(25))
	}

	fmt.Printf("OK local config chain MCP tool usable: tool=%s server=%s first_visible_prompt=%d\n",
		toolName, serverName, firstVisible)
	if notes := toolCallNotifications(a.updatesSnapshot()); len(notes) > 0 {
		fmt.Printf("OK ACP tool_call notifications: %s\n", strings.Join(notes, ", "))
	} else {
		fmt.Println("WARN no ACP tool_call notification observed for the MCP tool (upstream call still proved execution)")
	}
	if firstVisible == 1 {
		fmt.Println("G6: async-connected MCP tool IS visible at the FIRST prompt (measured)")
	} else {
		fmt.Printf("G6: async-connected MCP tool was NOT visible at the first prompt; first visible at prompt #%d (measured)\n", firstVisible)
	}
	fmt.Println("E2E PASS acp_e2e_mcp_local")
	return nil
}
