...
```powershell
aicli-mesh ls                  # 1) 现状：有哪些节点、谁活着、ADDR 是否为空
...
| 3 | 目标不可达 | `url` 目标无端点（纯 TUI）；`call` 的 `unreachable`/`timeout`；`stop` 的 `timeout`；`open` 的 `not_running` |
| 4 | 冲突或忙碌 | `call` 的 `busy`（目标已有 invoke 在等，HTTP 409） |
| 5 | 操作失败 | `gc --apply` 有删除失败项、`doctor` 发现问题、`open` 的 `failed`、JSON 输出失败、`call` 的 `error` |
...
### 4.2 拉起（`open` / `POST /web/api/mesh/spawn`）

| 码 | 退出码 | 含义与处置 |
|----|--------|------------|
| `mesh_spawn_bin_unavailable` | 5 | 找不到要拉起的 aicli 二进制：设 `AICLI_BIN` / `open --bin`，或与 `aicli-mesh.exe` 同目录放 `aicli.exe` |
...
| `mesh_workspace_missing` | 5 | 会话工作区不存在 |
...
| 列表里一堆 `stale` | 已退出进程的正常残留：`aicli-mesh gc --apply` |
...
| `call` 退出码 3 + `unreachable` | 档案还在但端口已关：目标已退出或未监听（`ls` 看状态、`doctor` 看 `stale-nodes`） |
...
aicli-mesh ls --json           # 空网格：counts 全 0、nodes 为 []（不是 null）
