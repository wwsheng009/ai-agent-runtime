...
- [mesh-cli.md](./mesh-cli.md)
  - Multi-process mesh operations CLI (`aicli-mesh`, also reachable as the `aicli mesh` alias — same implementation, same flags, same exit codes): `ls` / `show` / `url` / `call` / `send` / `screen` / `open` / `stop` / `watch` / `gc` / `doctor` / `version`, target resolution (node/session ID, ID prefix, `pid:<PID>` — ambiguity lists candidates instead of guessing), the stable `--json` schemas, exit codes 0–6, dry-run-first `gc` (live pids are never deleted), the `doctor` check set, the `AICLI_MESH_DIR` / `AICLI_HOME` layout, and the token policy (hint only; `url --with-token` is the single reveal path).
