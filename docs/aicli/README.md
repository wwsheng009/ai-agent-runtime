...
- [mesh-cli.md](./mesh-cli.md)
  - Multi-process mesh operations CLI (`aicli-mesh`, also reachable as the `aicli mesh` alias — same implementation, same flags, same exit codes): `ls` / `show` / `url` / `call` / `send` / `screen` / `open` / `new` (create a brand-new session; the child generates the session ID) / `stop` / `watch` / `gc` / `doctor` / `version`, target resolution (node/session ID, ID prefix, `pid:<PID>` — ambiguity lists candidates instead of guessing), the stable `--json` schemas, exit codes 0–6, dry-run-first `gc` (live pids are never deleted), the `doctor` check set, the `AICLI_MESH_DIR` / `AICLI_HOME` layout, and the token policy (hint only; `url --with-token` is the single reveal path).
  - 按主题组织的专题手册（快速上手 / 拉起与二进制解析 / 生命周期 / 排障）见 [../aicli-mesh/README.md](../aicli-mesh/README.md)。
