...
- 粘贴图片**路径**会自动成为附件：只在"整段粘贴就是一个存在的图片文件路径"时生效（Windows Terminal 把复制的图片文件转成路径、资源管理器拖拽路径都算），落附件后插入 `[Image #N]` 令牌；夹在句子里的路径、多行/多文件粘贴保持原样。注意 WT 自己处理 `Ctrl+V`/右键，位图请用 `alt+v`。
