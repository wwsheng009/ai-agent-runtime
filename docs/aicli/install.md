...
- Web 端的图片附件走同一套处理链（核心包 `internal/imageattach`），但**不使用 `[Image #N]` 令牌**：micro web client 用 `POST /web/api/attachments`（multipart，字段 `file`，一次最多 8 张）上传，返回的 `path` 由前端随 `POST /web/api/input` 的 `image_paths` 回传；React 前端用 `POST /api/runtime/uploads`（同一字段与张数上限）上传，随 `POST /api/runtime/sessions/{id}/runtime/commands` 的 `submit_prompt.images` 发送。两条路径的语义一致：上传**只落盘**（体积与真实格式判定→压缩→内容哈希命名，同一张图重复上传命中同一路径），**发送瞬间才登记**，因此"上传了没发"的图片不会粘进下一轮；被跳过的图片逐条给出原因（超体积上限 / 不是可识别的图片），绝不静默发送原图。服务端只接受自己产出的附件路径（runtime 侧还会做附件目录边界校验），路径外的 `images` 会被拒绝并说明原因。
