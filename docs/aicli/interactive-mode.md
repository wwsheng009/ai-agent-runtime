...
- **发送前图片处理**（`/attach <path>` 与剪贴板图片共用）：长边超过 `1568px` 的等比缩小且只缩不放，含透明通道保持 PNG、其余转 JPEG；超过 `32MB` 的直接跳过。两者都**不静默**——会写明「已压缩 3000x2000 → 1568x1045（3.2MB → 420KB，JPEG）」或「已跳过 …：33.0MB 超过 32.0MB 上限（图片不会随消息发送）」。压缩产物按内容哈希命名并落在会话 `images` artifact 目录，同一张图重复粘贴不会重复入列。
  ...
  - 上限优先级（四条图片入口共用）：环境变量 `AICLI_IMAGE_MAX_DIMENSION`（`0` 关闭压缩、负数不缩放，仍做体积上限检查）> 配置文件 `aicli.chat.max_image_dimension`（同语义）> 内置默认 `1568px`；体积上限同理 `AICLI_IMAGE_MAX_MB` > `aicli.chat.max_image_mb` > 默认 `32MB`（配置项 `<=0` 或非法值一律回落默认；不提供"完全不限制体积"的取值，避免误发超大原图）。
- **图片令牌 `[Image #N]`**：附件成功后会插入一个可见令牌（`alt+v` 在光标处插入；键入的 `/attach <path>` 在命令执行后写回下一次输入框草稿），编号即 `/attach` 列表里的序号。
  ...
  - **粘贴图片路径也会成为附件**：整段粘贴内容由一个或多个图片文件路径组成时（Windows Terminal 把复制的图片**文件**转成路径注入、资源管理器拖拽路径同样是路径，多选则是多个路径），走与 `/attach` 同一条管线（校验 → 压缩/上限 → 去重）落附件，并把 `[Image #N]` 令牌按粘贴顺序插到光标处，而不是把路径当文本贴进去。
    - 判定刻意保守：整段文本必须**只由路径与空白组成**（出现"请看"这类碎片就整体不命中），未加引号的标记不含空白（含空格路径由 WT/拖拽加引号），每个标记的扩展名都要在候选集合内；引号未闭合、引号后紧跟内容都算不命中。同一路径重复出现会先去重。
    - 多路径的**一致性规则**：校验阶段**全有或全无**——任一标记不是可用图片就整体回落为文本粘贴，避免"一半转了附件、一半还是路径"；压缩/体积上限这类按规则跳过**不阻塞**其它图片，能加的照加、跳过的在状态行如实写明原因。
    - 一次粘贴最多转换 **8** 张（超出则整体回落并提示，避免粘贴一堆路径时长时间卡住输入、撑爆请求）；夹在句子里的路径、非图片内容完全静默，只在"像图片路径但没能加成附件"时给一行状态提示。
