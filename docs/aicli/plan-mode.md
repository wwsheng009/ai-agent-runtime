...
4. **接线已全部落地**（HTTP / CLI / Web 三面）：
   * **CLI**（与 `/plans diff` 一样直接读写归档存储，不经 HTTP）：`/plan comment <Lx[-Ly]> <正文>`、`/plan comments`。**首次评论时若记录还没有归档轮次，会先把当前计划正文按 `decision=comment` 登记为 v1**（`archiveStatus` 对 `comment` 保持原状态，只补快照）——锚点必须指向用户真正看到的正文，宁可多一轮也不猜行号；正文还没写入工作区则直接报错。`/plan request_changes` 时评论并入一次性评审提醒（§4 第 3 条），而轮次记录里仍只存用户自己写的 notes（评论有独立日志，避免同一内容两处漂移）。列表**不隐藏失效锚点**：`[与当前正文一致]` / `[原文已移动（原 L3-4）]` / `[锚点失效（该处正文已改写，保留当时内容）]` 三态并列显示。
   * **Web**（§6 三条端点的消费面）：轮次差异视图的行号可点——点击选行、Shift+点击扩区间，**行号取的是该 diff `to` 版本的正文行号**（由 hunk 头 `@@ -a,b +c,d @@` 起算，不是 diff 文本行序），提交时带 `revision=to`，锚点因此落在用户真正看到的那一版；评论列表按最新轮重放，直接消费 `status`/`current_*` 投影（前端不重算锚点），三态并列 + 可删除。
...
- §4.4 行级评论与轮次 diff：**已全部落地** —— 轮次 diff（`/plans diff <id> [vA [vB]]` + `GET /plans/{id}/diff` + 面板评审轮次行的「差异」展开，同一 `planmode.DiffArchivedVersions` 口径，新增行 teal / 删除行 orange，`identical` / `coarse` / `truncated` 有独立徽标）；行级评论四层齐备：存储（`planstore` 的 `comments/` 日志）、锚点重放（`planmode` 的创建/重放/渲染）、HTTP（§6 三条端点）、CLI 与 Web 入口（§4 第 4 条）。**该小节的缺口已关闭。**
...
- 评审反馈的**自动修订回合**：**已落地（§4 第 4 条）** —— HTTP `trigger_revision=true`、Web 面板「请求修改」、交互式 CLI `/plan request_changes <notes>`（统一 TTY 走 `SendMessageAfterCommit` post-commit 边界，纯文本 REPL 在裁决行后直接提交）都在裁决落地后立刻起一轮；脚本 / JSON 仍是「下一次用户输入时交付」。**§4.6 已全部关闭**；**§4.4 也已全部关闭**（行级评论的存储、重放、HTTP、CLI 与 Web 五面齐备，见 §4 契约块与 §6）。
