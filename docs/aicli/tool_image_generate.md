...
通过普通 chat 工具调用（模型自动调用或 `/call openai_image_generate`）时会保存到当前会话的 `generated-images` 目录；`aicli image` 子命令与 chat 内 `/image` 快捷命令默认保存到当前工作目录下的 `generated-images`，也可用 `--output-dir` 覆盖。
...
进入 `aicli chat` 后，可以用 `/image` 作为 `aicli image` 的 chat 内快捷入口。它复用同一套参数解析、provider 选择、failover、请求重试、图片保存和 metadata 结构：

```
/image 一只在月光下奔跑的猫
/image --provider SENSENOVA_IMAGE --output-dir ./generated-images 生成一张产品海报
/image --provider CODEX_04 --model gpt-5.4 --path codex_native 生成一张未来城市海报
/image --json 生成一张壁纸
```
...
/image --provider SENSENOVA_IMAGE 测试
...
/image --model sensenova-u1-fast 测试
...
/image --provider SENSENOVA_IMAGE --size 2752x1536 测试
...
| Chat 快捷命令 | `backend/cmd/aicli/commands/chat_image_command.go` | `/image` 参数解析并复用 `runImageGenerateCommand` |
