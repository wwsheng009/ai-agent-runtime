...
| 4.4 第二步·发送前压缩与上限 | 新增 `internal/imageprep`：长边上限（默认 1568px，只缩不放）+ 体积上限（默认 32MB，超限标记 `Skipped` 并给原因），透明保 PNG、其余 JPEG（q85），面积平均（box filter）缩放；长边上限是**硬规则**（扁平/合成大图重编码变小也照样缩，提示如实写出体积变化）；产物按**内容哈希**命名落在会话 `images` artifact 目录，同图重复粘贴天然幂等。接线：`/attach <path>` 与剪贴板图片共用的 `prepareChatImageAttachment`，`AICLI_IMAGE_MAX_DIMENSION` 可覆盖（0 关闭、负数不缩放） | `internal/imageprep/*`、`commands/chat_attach_image.go`、`chat_clipboard_image.go`、`command.go`/`chat_command_result.go`（`/attach`） | `imageprep_test.go`（缩放尺寸、不放大小、透明度选格式、超限跳过、坏图报错、同内容幂等、`TestDownscaleAveragesBlocks` 四色块精确校验、`TestPrepareResizesEvenWhenReencodeGrows` 硬规则回归）、`chat_clipboard_image_test.go`（压缩落 artifact、超限只提示、关闭开关、`/attach` 压缩、env 映射表） |
| 4.4 第三步·图片内联令牌 | 引入 `[Image #N]` 令牌：`alt+v` 在光标处插入令牌，键入的 `/attach <path>` 在命令执行后把令牌写回下一次草稿；提交时按草稿里存活的令牌裁剪附件（**删令牌即弃图**）。只约束"由令牌引入"的附件（`session.imageTokenPaths` 记录路径→序号），ACP/Web/resume 来源的附件不受影响；草稿里完全没有令牌时不做裁剪（保守，不静默丢图）；附件移除/发送成功/会话重置时同步清理标记。实现需要让动作键也能改写当前行：`ui.LineEditorHooks.OnActionKey` 由 `(bool,bool)` 改为返回 `LineEditorActionResult{Claimed, ExitEditor, Replacement}`，编辑器在动作键上应用替换 | `commands/chat_image_tokens.go`、`chat_composer.go`（动作键插令牌 + 提交裁剪）、`chat.go`（命令后写回草稿）、`chat_send.go`/`chat_session.go`/`command.go`/`chat_command_result.go`（标记清理）、`ui/inputbox_editor_hooks.go`、`ui/inputbox_editor.go` | `chat_image_tokens_test.go`（解析/插入/裁剪表 + 动作键插令牌 + 提交裁剪 + 命令后写回）、`ui/inputbox_keymap_test.go`（动作键替换改写当前行）、既有 clipboard/composer 用例适配新签名 |
| 4.4 第四步·发送时对齐 | `sendMessage` 入口做**一次**令牌/附件对齐（`normalizeChatTurnImagePrompt`）：按令牌出现顺序重排附件并把令牌重编号 `1..k`（文本顺序 == 发送顺序）；丢弃没有对应附件的悬空令牌并收拢遗留空格；非令牌来源附件追加末尾且不补写令牌；无令牌或令牌全悬空时保守原样返回（不改写用户文字）。因为对齐发生在 turn 入口，**记录的用户消息、模型 prompt、附件列表**三处天然一致 | `commands/chat_image_tokens.go`、`commands/chat_send.go:sendMessage` | `chat_image_tokens_test.go`：8 例对齐表（顺序一致/重排重编号/悬空丢弃/未引用不发送/外部追加/全悬空保守/重复引用）+ 会话级标记重建 + 无令牌不受影响 |
| 4.4 第五步·ctrl+v 图片兜底 | 新增 `ui.LineEditorHooks.OnClipboardTextEmpty`：`ctrl+v` 读不到剪贴板文本时给宿主一次接管机会，编辑器只在宿主返回 `Replacement` 时改写当前行，零值则保持原有静默行为。composer 把 `alt+v` 与兜底合并到同一实现 `attachClipboardImageToLine(snapshot, quiet)`：成功都落附件 + 插令牌 + 状态行提示；`quiet=true`（ctrl+v）时**所有失败路径静默**，`quiet=false`（alt+v）时才报错。剪贴板同时有文本和图片时仍优先粘贴文本，避免改变既有语义 | `ui/inputbox_editor_hooks.go`、`ui/inputbox_editor.go`、`commands/chat_composer.go` | `ui/inputbox_keymap_test.go`（ctrl+v 无文本 → 兜底改写提交行；无钩子 → 保持静默）、`chat_image_tokens_test.go`（兜底读图插令牌；无图片 → 零值且不新增附件） |
| 4.4 第六步·粘贴图片路径识别 | 新增 `ui.LineEditorHooks.OnPasteText`（编辑器**唯一**粘贴入口，覆盖 bracketed paste / ctrl+v 文本 / 粘贴突发），宿主返回 `Replacement` 时编辑器不插入原文本。composer 侧 `chatPastedImagePath` 保守判定"整段粘贴 = 单个图片文件路径"（单行、允许成对引号、无引号时不得含空白、扩展名在候选集合内、去外层引号后不得再有引号 ⇒ 多文件不处理），命中后走既有 `/attach` 管线（`ValidateLocalInputImagePaths` → `prepareChatImageAttachment` 压缩/上限 → 去重）并复用 `chatImageTokenReplacement` 插令牌 + 标记（删令牌即弃图）。动机：Windows Terminal 自己处理 `Ctrl+V`/右键并把复制的图片**文件**转成路径注入，按键与剪贴板钩子都看不到，只能在粘贴文本层面识别。所有失败路径回落到原样粘贴，只有"像图片路径但没加成"才给状态行提示 | `ui/inputbox_editor_hooks.go`、`ui/inputbox_editor.go`、`commands/chat_paste_image_path.go`、`commands/chat_composer.go` | `ui/inputbox_keymap_test.go`（bracketed paste 命中改写 / 未命中原样插入）、`chat_image_tokens_test.go`（10 例判定表 + 含空格引号路径落附件插令牌 + 重复粘贴去重 + 5 例回落） |
| 4.4 第七步·多文件/多行路径粘贴 | 把第六步的"整段粘贴 = 单个图片路径"扩展为**一个或多个**：新增 `splitPastedPathTokens`（空白/换行分隔、成对引号内整体保留；未闭合引号、引号后紧跟内容、标记内混入引号一律不命中）与 `chatPastedImagePaths`（每个标记都要过扩展名候选集、忽略大小写去重）。一致性规则：**校验全有或全无**（任一不是可用图片 ⇒ 整体回落为文本粘贴，不留半吊子状态）；压缩/体积上限这类按规则跳过**不阻塞**其它图片，跳过的在状态行写明原因；一次最多 `maxChatPastedImagePaths=8` 张（超出整体回落 + 提示）；令牌按粘贴顺序插入，编号复用既有附件。第六步的"多文件不处理"由本步取代 | `commands/chat_paste_image_path.go` | `chat_image_tokens_test.go`：判定表扩到 15 例（同行多路径/多行/去重/未闭合引号/引号后紧跟内容/混入非图片）、多图落附件 + 顺序令牌 + 标记、多图重复粘贴去重、数量上限回落、6 例回落表 |
| 4.4 第八步·上限接入配置 | 新增 `aicli.chat.max_image_dimension`（像素；`0`/负 = 关闭压缩/不缩放）与 `aicli.chat.max_image_mb`（MB；`<=0` = 默认 32MB），解析优先级 **环境变量 > 配置文件 > 内置默认**（新增 `AICLI_IMAGE_MAX_MB`，`AICLI_IMAGE_MAX_DIMENSION` 保持既有语义），且 `max_image_mb` 不接受"完全不限制体积"的取值。`prepareChatImageAttachment` 改为按会话解析上限，`/attach`、`alt+v`、`/attach paste`、路径粘贴四条入口同时生效 | `internal/agentconfig/config.go`（`AICLIChatConfig` 两个指针字段）、`commands/chat_attach_image.go` | `chat_image_limits_test.go`：长边 8 例优先级表（含非法 env 回落配置、0=关闭、负数不缩放）+ 体积 7 例表 + YAML 键名锁死 + 配置值真正触发缩放（2×2 → 1px，格式无关解码）；既有 `TestChatImageMaxDimensionMapping` 适配新签名 |
| 4.4 仍未做（明确遗留） | ① 可点击预览（终端不具备） | — | — |
