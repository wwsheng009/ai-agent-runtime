...
### 18.6 环境提示：索引快照事件与修复（2026-09-25）

本轮两次提交（`15ea096e` / `9647034b`）落地后复查，发现**真实 `.git/index` 里我的 15 个文件被写回了旧版本**：6 个新文件变成「暂存删除」（`D `），9 个改动文件成 `MM`，`git diff --cached --stat` 显示 `20 insertions / 606 deletions`。

- **内容未受损**：逐文件 `git hash-object --path=<p>` 与 `HEAD:<p>` blob 比对，15/15 一致；文件都在磁盘上，只是被索引当成了「已删除 + 未跟踪」。
