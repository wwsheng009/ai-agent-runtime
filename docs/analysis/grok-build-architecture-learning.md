...
> R1–R7 已收口（permissions 产品面 / folder trust / system-reminder / teammate↔agentdef / frontend harness 面板 / ACP session/load / tool terminal stream）；A5 residual（2026-07-26）补齐 complete_task 缺失时 structured JSON fallback → blocked/done 恢复；R3 residual（2026-07-26）pure advisory strip-on-persist；旁路产品面 **user-turn backtrack Phase 1–6 + residual UI done**；旁路 **STALE_CONTEXT pipeline residual done**（2026-07-26：tool-authored code 保留 + disposition 提升 + Diagnose nested next_action + chat-log 导出 `failure_class`/`suggested_view_*` + edit whitespace auto-heal + **edit/apply_patch current_snippet copy-paste recovery**）；旁路 **adaptive thinking wire residual done**（2026-07-26：`thinking.adaptive.effort` 不再嵌套序列化，仅 `output_config.effort` + ReasoningEffort 派生路径降级）；旁路 **failed/STALE disposition replay residual done**（2026-07-26：failed disposition 跟踪 + STALE 专属 advisory + dashboard rehydrate `STALE_CONTEXT`）；旁路 **path_missing unique auto-heal residual done**（2026-07-26：read-safe preflight 唯一高置信 sibling 改写 + `path_auto_healed` metadata + PATH_NOT_FOUND disposition advisory）；旁路 **path candidate quality residual done**（2026-07-26：过滤 `.backups` 噪声 + parent discovery `path_candidates` + PATH next_action）；旁路 **path separator-fold auto-heal residual done**（2026-07-26：`providertoken`↔`provider_token` 等 `_`/`-` 折叠唯一 sibling 改写 score=85）；旁路 **path placeholder empty residual done**（2026-07-26：`path=""`/`null` 占位归一，禁根目录噪声 candidates）；旁路 **edit STALE mislabel refine residual done**（2026-07-26：edit 家族 TIMEOUT/PATH 误标→`STALE_CONTEXT`，message 分类 STALE 优先于 PATH/TIMEOUT）；旁路 **apply_patch blank-run residual done**（2026-07-26：`locateBlankRunLineSpan` 共享 + hunk 柔性定位 + file blank-run 保留替换）；旁路 **STALE recovery field export residual done**（2026-07-26：error body 回填 `current_snippet` + ApplyDiagnosticMetadata/tool.completed/protocol thin metadata 导出）；旁路 **edit column/blank-run + model contract current_snippet residual done**（2026-07-26：collapse-ws/blank-run heal + contract 导出 current_snippet 文本 4KiB 整行 cap）；旁路 **web_search network fail residual done**（2026-07-26：connectex/dial → `NETWORK_UNAVAILABLE` + query-aware next_action，HTTP 5xx/429 优先）；旁路 **dashboard STALE snippet rehydrate + path ambiguity residual done**（2026-07-26：body 回填 `current_snippet` 收紧 header/numbered-line；歧义 path next_action Pick exactly one）；旁路 **STALE closest near-identifier residual done**（2026-07-26：`runeEditSimilarity` 软匹配 HelloWord↔HelloWorld + structural 降权 + apply_patch winning-expected tighten / 右偏 pad）；旁路 **Chinese invalid-args next_action residual done**（2026-07-26：toolkit 中文「参数缺失/无效」→ 具体 required/type next_action）；旁路 **parent sibling discovery residual done**（2026-07-26：parent-only PATH fail 附非噪声 sibling 样本到 path_candidates）；旁路 **shell multi-key timeout_ms=1 residual done**（2026-07-26：`timeout_ms=1`+`timeout_sec` schema 噪声协调，丢弃 <100ms 假超时）；旁路 **true content_diff / 旧二进制无 closest residual done**（2026-07-26：edit token/single-line closest fallback + apply_patch full-line `N|` 块 + 截断 %q rehydrate）；旁路 **apply_patch invalid syntax / path crumb residual done**（2026-07-26：非法 hunk/信封 → TOOL_INVALID_ARGS + sanitizeApplyPatchPath 剥离路径尾部引号标点）；下一波 ROI 候选为 R8+（可选重开项：FTS/marketplace/best-of-n）或会话效率方案中仍可压的 hard-rate（真 semantic drift 后模型是否 copy-snippet / path / shell 次峰）/ 协作体验项。
