...
> Harness：`scripts/test-aicli-debug-endpoints-e2e-mesh.ps1`（M1–M10；含启动自检与清单覆盖门禁共 13 条断言，基线已固化）
...
> 原 §8.1 → 本文 §1、原 §8.5（M1–M10 断言表）→ 本文 §5、原 §8.8（安全红线）→ 本文 §8。
...
> [../plan/aicli-mesh-implementation-plan.md](../plan/aicli-mesh-implementation-plan.md) §15.3 的 D5–D12
