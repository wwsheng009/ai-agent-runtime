...
| **P2** | S10+（接管 / 工作区收敛开关 / stop / journal 查询） | ① 冲突详情横幅（节点列表 + 心跳）<br>② 接管二次确认（`takeover`）<br>③ 收敛开关（`--mesh-restrict-workspace`，opt-in）下的 `refused` 文案（`mesh_cross_workspace_denied`）<br>④ resume 的 SSE 事件化（去 8×300ms 轮询）<br>⑤ 窗口标题加节点后缀 | E2E-DEBUG-03 + 手工 |
