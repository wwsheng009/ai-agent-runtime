...
> `internal/mesh/spawn_test.go`、`cli_open_test.go`；E2E-DEBUG-03 的 M1–M12 覆盖**网格行为**
...
### 10.3 E2E 验收（v1 E1–E8 → 网格 M1–M12 映射）
...
> M11/M12 是**治理面**断言（非回环默认拒绝 / journal 降级），与 E1–E8 无一一对应关系，
> 见 [mesh-e2e.md](../e2e/mesh-e2e.md) §5。
...
| §12 测试与验收 | 本文 §10（重写） | 单测改为 Web 侧契约；E1–E8 → 网格 M1–M12 映射；回归清单更新 |
