...
> **当前施工基线（2026-08-06，覆盖后文旧阶段快照）**：生产 primary 已改为 `top native-history region + bottom inline viewport`。finalized transcript 的全部 display rows 和 mutable active 的稳定 overflow prefix 通过 reducer-owned `HistoryCommit` 插入 `1..OutputBottomRow`；`ScreenModel` 只能持有 `OutputBottomRow+1..terminalHeight`，不能重绘 history。finalized plain source 的内部空行与 trailing empty row 均持有稳定 `SourceRange/FragmentID`；finalize 只补交尚未 Ack 的 tail，绝不重放 active 的已 Ack prefix。active 使用 `Stable/Enqueued/Acked` 连续 source range，只有 Ack 才从 live tail 隐藏；Markdown handoff 保持结构化 IR。native scrollback 首次溢出后，resident tail 必须 sticky top-aligned，scrollback/viewport 边界不得产生非语义空白断层。一次事务顺序固定为 viewport boundary transition、history insert、viewport diff、cursor。零字节失败在 recovery 后重试同一 token，partial write 保持 Unknown 且不得盲重试。

> **验证基线**：`go test ./cmd/aicli/... -count=1`、`go test -race ./cmd/aicli/ui ./cmd/aicli/commands -count=1`、`go vet ./cmd/aicli/ui ./cmd/aicli/commands` 均通过。synthetic Windows Terminal E2E 已验证 72 条增量 history exactly-once，并验证 committed Markdown 在宿主 `DocumentRange` 中只出现渲染后的 heading/emphasis/code 内容、不泄漏 raw Markdown 标记。真实 provider E2E manifest 位于 `output/aicli-terminal-e2e/opencode-wt-7347d2bf0b0346c1ba975085b3c8b2eb/manifest.json`：稳定 UIA `DocumentRange`/`VisibleRanges` 均已采集，40 个 marker 恰一次、严格有序且连续；chat artifact 中唯一 reasoning summary 的完整非空白内容在宿主文档中恰好匹配一次并位于首个 final marker 前，且无 `assistant.reasoning`、`llm.request.started`、`llm.request.finished` 裸协议标签。退出由按 executable path + run ID 精确定位的独立 `AttachConsole+WriteConsoleInputW` helper 发送，helper/runner code 均为 0、forced cleanup 为 0。此 run 不等同于 compatibility cleanup、全宿主矩阵或长会话验收完成。后文 whole-screen、retained-frame 或未安装 seam 的文字仅为历史，不得恢复。
...
- **遗留边界（2026-09-01 已关闭）**：`renderStatusLocked`/`renderPopupLocked`/`renderPromptRowsLocked` legacy 即时渲染主体已删除（替换为 Phase 6 注释占位），配套 33 个 legacy 合约测试已迁移到 owned 模式（`newOwnedTestFixedBottomSurfaceWithSize` + owned diff 输出契约断言），`go test ./cmd/aicli/... -count=1` 全绿；`screen_lease.go` 的 Release legacy 渲染序列经核对仍为 owned/lease 模式活跃代码（保留）。详见母计划"切片 17 施工记录续"。
...
真实 provider 脚本不得通过 `SendInput`、剪贴板或 UI Automation 注入测试 prompt 或 `/exit`。runner 生成的命令行必须显式包含单一 `--prompt`；退出必须由按 executable path + run ID 精确定位的独立 `AttachConsole+WriteConsoleInputW` helper 发送，并且仅在 helper code 为 0、runner code 为 0、forced cleanup 为 0 且退出码文件出现后才报告完整 PASS。
