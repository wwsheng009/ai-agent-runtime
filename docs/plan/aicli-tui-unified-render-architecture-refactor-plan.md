...
> **2026-08-06 权威验收结论（覆盖本文所有早期阶段快照）**：核心 inline rendering 目标已经在 production interactive 路径落地并通过一次真实 provider + Windows Terminal E2E。验收 run 为 `output/aicli-terminal-e2e/opencode-wt-7347d2bf0b0346c1ba975085b3c8b2eb/manifest.json`：稳定 UIA `DocumentRange`/`VisibleRanges` 均已采集，40/40 assistant marker 在 `DocumentRange` 中各出现一次且严格有序、连续，marker 01 已滚出 visible range 但仍可从 native scrollback 读取，marker 40 可见；chat artifact 中唯一 reasoning summary 的完整非空白内容已在宿主文档中恰好匹配一次且位于首个 final marker 前；`assistant.reasoning`、`llm.request.started`、`llm.request.finished` 裸协议标签均未泄漏；异常非语义空行数为 0。`/exit` 由按 executable path + run ID 精确定位的独立 `AttachConsole+WriteConsoleInputW` helper 发出，helper/runner code 均为 0、forced cleanup 为 0。synthetic Windows Terminal E2E 另验证了 72 条增量 history exactly-once 与 committed Markdown 的宿主级渲染。该 run 使用 `--prompt` 自动提交首轮，并验证真实 `opencode.ai/deepseek-v4-flash/max` 路由 artifact；它不代表 compatibility cleanup、全宿主矩阵或长会话验收已经完成。
...
> **2026-08-05 cutover 更正**：上方 Phase 2、history-effect 与 `TerminalSession seam` 三段均是 pre-cutover 的历史增量，不能再用于判断当前 primary。现在 owned interactive 的 `TerminalSessionPresenter` 已消费同一 `AppState -> Layout/Compose` frame，并在同一 terminal transaction 内执行 `HistoryCommit` handoff、viewport diff 和 cursor；`FixedBottomSurface` 的 physical writer 与 legacy history handoff 已 fence。尚未完成的是 AppState 的全 producer/interaction 覆盖、compatibility state 删除、跨 Markdown/超长单行的剩余 source-fragment 边界，以及除已通过 Windows Terminal/ConPTY E2E 外的 PTY/ConPTY 宿主与长会话验收。

> **Resume picker typed-effect 切片（2026-08-05）**：`/resume` 与 `/resume --cwd` 在兼容 ANSI TTY 上均不再进入 legacy line-reader。它先以携带不可变 `ResumePickerRequest{Filter}` 的 `CommandResult.OpenResumePicker` 请求 effect，再由 `OpenResumePicker`/`CloseResumePicker` UI actor barrier 绑定同一 `ScreenLease`；列表所有 alternate-screen 字节经 `TerminalSessionPresenter` 传输。选择、取消或错误只有在 lease 释放和 primary recovery barrier 后才追加 command cell；成功选择再以 `ReplayHistory` 回放 canonical transcript。非 ANSI/非 TTY 环境或 lease/popup 忙时保持 fail-closed，绝不回退到 legacy stdin/stdout。

> **Backtrack picker + canonical replacement 切片（2026-08-05）**：bare `/backtrack`、`/backtrack select|pick|ui` 在兼容 ANSI TTY 上以 `CommandResult.OpenBacktrackPicker` 进入 lease-bound fullscreen effect。选中存在后续文件变更的 turn 时，第二个范围列表在**同一** lease 内选择“仅对话”或“对话和代码”；不再读取 legacy stdin。`CloseBacktrackPicker`、`LeaseReleased` 和 primary recovery barrier 完成后才调用 actor 的 destructive apply。成功 apply 不走 append-only history replay：它清空 encoder/Scene 派生投影，以截断后的 canonical history 重建 cell，并追加一条 history-reset event-log marker；下次日志 replay 遇到 marker 会丢弃此前已被截断的 event projection，防止旧 turn 复活。随后才提交 result command cell，再通过 actor 恢复 composer draft。直接 `/backtrack <N> --apply|--submit` 现以 `CommandResult.ApplyBacktrack` 进入**同一** destructive transaction：先 actor apply/sync/replacement，再单个 result cell，最后才恢复草稿或走标准 send pipeline；不产生预 mutation cell，也不回退 stdout。numeric `/rewind` 继续 fence，直到 checkpoint restore 拥有独立 typed transaction。

| 阶段 | 当前状态 | 已验证证据 | 仍待完成 |
| --- | --- | --- | --- |
| P0 旁路审计 | **进行中** | `TestChatInteractiveDirectWriterInventory` 以 AST 扫描 `commands/chat*.go` 与 `command.go`，固化 **145 个 grouped debt entries / 447 个 call sites**（391 `fmt.Print*`、46 `fmt.Fprint*(os.Std*)`、1 `io.WriteString(os.Std*)`、9 `ui.WriteTerminal*`）；新增 group 或 count 变化默认使测试失败，并报告实际行号。完整 `/debug`、`/session`、`/status`、`/load`、直接 `/resume` target（`latest`/显式 ID）、`/goal`、`/memory`、`/stream`、`/fast`、`/reasoning`、非选择器 `/reasoning_effort`、`/compact`、`/retry`、`/model status`、只读 `/theme`（`status`/`list`/`preview`/`syntax`）、显式 `/skills <query|list>`、只读 `/backtrack`（`list`/`audit`/numeric preview）、`/plan`、`/timeline`、静态 `/collab`、静态 `/agents`、`/trust`/`/trust status` 与 `/title`/`/rename` 成功路径已有 raw-stdout/structured producer fence 及 atomic command-cell 回归；`TestChatSystemOutputWriter_ActiveTurnMirrorSurvivesOwnedViewportRepaint` 覆盖 active-turn mirror 在 status/ActiveBand 重绘后仍只出现一次。`functions/builder.go` 的遗留 function-call parser 已删除全部 direct writer，并有 source fence 与 malformed-call 语义测试。 | 每条 chat/command debt entry 标注 owned-safe/plain-only/startup-shutdown/待迁移 owner；补 owner/frame trace；迁移后逐项从基线删除。 |
| P1 CommandResult | **部分完成** | 完整 `/debug`（status/on/off/routing/display/export）、`/session`、`/status`、成功 `/load <session-id>`、直接 `/resume latest|<session-id>`、`/goal`（status/clear/pause/resume/complete/set）、`/memory`（status/add/list/search）、`/stream`（status/toggle/set，含 `/s`、`/n`）、`/fast`、`/reasoning`、非选择器 `/reasoning_effort`（status/clear/explicit set）、`/compact`、`/retry`、`/model status`、只读 `/theme`（`status`/`list`/`preview`/`syntax`）、显式 `/skills <query|list>`、`/backtrack` 的只读查询与 direct `--apply|--submit`、`/plan`（status/enter/exit）、`/timeline`、静态 `/collab`、静态 `/agents`、`/trust`/`/trust status` 与成功 `/title`/`/rename` 已以结构化 `CommandResult` 进入单个 command cell（另有 `chat_title_document.go`）；theme preview 保留 `render.Document` 的 syntax/diff spans，禁止把 ANSI 字节写入 retained cell。handler 禁止 direct terminal writer 的测试已覆盖全部新式 producer。`/goal <objective>` 在确认 cell 提交后经 `CommandResult.SendObjective` 走正常 send pipeline 触发 AI 目标请求；`/retry` 的结果 cell 提交后经 `CommandResult.RestoreComposerDraft` 由 actor 恢复草稿，且若已有新草稿则追加拒绝 cell，永不覆盖输入或自动重试；`/backtrack <N> --apply|--submit` 在 apply/sync/canonical replacement 后才写入单个 result cell，并在其后恢复草稿或走正常 send pipeline；structured `/debug`、`/stream`、`/fast`、`/reasoning_effort`、`/plan` 的 runtime session 同步、配置持久化或 archive 失败会追加同一 cell 内的 warning/错误，禁止 stderr 旁路；`/reasoning_effort select` 继续由 unified gate 拒绝，直到 picker 迁移为 lease/UI-action；bare `/model`、model mutation、bare `/theme`、theme selection/mutation、bare `/skills` 的 selection/execution 继续拒绝，直到具备对应 typed picker/mutation/execution effect 与 presenter-owned full repaint transaction；numeric `/rewind` 也保留在 checkpoint restore namespace 的 fence 内；`/collab follow` 保持结构化拒绝，直到具备 typed effect/result stream；`/agents` 的 pick/send/target/panel/routing 保持结构化拒绝，直到各自具备 typed input/effect；`/trust grant` 保持结构化拒绝，直到具备 typed confirmation effect；title mutation 与成功确认 cell 共用 side-effect boundary。仍包含 modal/fullscreen、`--json` 或复杂副作用的命令保持 fail-closed/unified gate；成功 `/load` 与直接 `/resume` 都在确认 cell 后通过 `CommandResult.ReplayHistory` 逐消息回放历史。 | 迁移 `/skills` 的 selection/execution（留待 order-3 桶）及其余 slash command，删除 `beginDirectInteractiveOutput` 作为通用命令协议。 |
...
- 当前基线为 145 个 file/function/kind 分组、447 个 call site；新增分组或调用次数变化均默认失败，失败信息包含实际行号；
- 完整 `/debug`、`/session`、`/status`、`/load`、直接 `/resume latest|<session-id>`、`/goal`、`/memory`、`/stream`、`/fast`、`/reasoning`、非选择器 `/reasoning_effort`、`/compact`、`/retry`、`/model status`、只读 `/theme`（`status`/`list`/`preview`/`syntax`）、显式 `/skills <query|list>`、`/backtrack` 的只读查询及 direct `--apply|--submit`、`/plan`、`/timeline`、静态 `/collab`、静态 `/agents`、`/trust`/`/trust status` 与 `/title`/`/rename` 的已迁移路径验证不写 raw stdout、各作为一个 atomic command cell 提交，并在 prompt/status/ActiveBand repaint 与 resize recompose 后不重复或丢失；`/theme preview` 的 syntax/diff spans 在 `render.Document` 中保留到 presenter 的最终主题解析，禁止在 retained cell 中保存 ANSI 字节；`/retry` 恢复草稿经 command-cell post-commit actor action，有新输入时拒绝覆盖；结构化 `/debug`、`/stream`、`/fast`、`/reasoning_effort`、`/plan` 的同步、持久化或 archive 失败必须在同一 command cell 中显示 warning/错误，不能回落 stderr；`/collab follow`、`/agents` 的交互子命令、`/trust grant`、bare `/model`、theme picker/mutation、bare `/skills` 的 selection/execution 及 numeric `/rewind` 明确拒绝，不能回落旧轮询、输入、确认、history truncation 或直接刷新输出。
- `/resume` 与 `/resume --cwd` 在可用 fullscreen TTY 上已改为 `CommandResult.OpenResumePicker(ResumePickerRequest{Filter}) -> OpenResumePicker/CloseResumePicker -> ScreenLease` 的 typed effect；`TestResumePickerReducer_BindsAndClearsLeaseOwnership` 固化 stale open/close 不得窃取 lease，`TestResumePickerRequestCarriesCwdFilter` 固化 workspace filter 不会在 dispatch 时丢失。该路径没有 plain fallback；无 alternate-screen 能力或 lease/popup 忙时继续作为语义 fence cell。
- `/backtrack` 的 picker 已改为 `CommandResult.OpenBacktrackPicker -> OpenBacktrackPicker/CloseBacktrackPicker -> ScreenLease`；direct `/backtrack <N> --apply|--submit` 已改为 `CommandResult.ApplyBacktrack`，复用 actor apply -> CLI sync -> canonical replacement -> one result cell -> draft/normal send 的顺序。`TestBacktrackPickerReducer_BindsAndClearsLeaseOwnership` 固化 lifecycle ownership，`TestApplySelectedBacktrackReplacesUnifiedCanonicalTranscript` 与 `TestBacktrackQueriesStayOnUnifiedTerminalSession` 覆盖截断后旧 query cells 被 canonical replacement 清除，`TestStructuredBacktrackApplyCarriesTypedMutationEffect` 固化 dispatch 不生成 pre-mutation document，`TestReplaceCanonicalHistoryProjectionDiscardsTruncatedTurnsAcrossEventLogReplay` 覆盖 history-reset marker 在 restart replay 时删除旧 event projection。无 alternate-screen 能力、lease/popup 忙或 active run 的 picker 继续 fail-closed；numeric `/rewind` 保持独立 fence。
...
2. `/debug`、`/session`、`/goal`、`/memory`、`/stream`、`/fast`、`/reasoning`、`/reasoning_effort`、`/compact`、`/retry`、`/model`、`/theme`、`/plan`、`/timeline`、`/collab`、`/agents`、`/trust`、`/skills`、`/backtrack`、`/resume` 等文档型输出（完整 `/debug`、`/session`、前五者及 `/s`、`/n`，以及非选择器 `/reasoning_effort`、`/compact`、`/retry`、`/model status`、只读 `/theme`、显式 `/skills <query|list>`、`/backtrack` 的 `list`/`audit`/numeric preview、picker 和 direct `--apply|--submit`、直接 `/resume latest|<session-id>`、`/plan` 的 status/enter/exit、只读 `/timeline`、静态 `/collab`、静态 `/agents` 与只读 `/trust` 已迁移；`/theme preview` 使用结构化 preview document 而不是 ANSI 字符串；`/collab follow`、`/agents` 交互子命令、`/trust grant`、model/theme picker 或 mutation、bare `/skills` 的 selection/execution 仍待迁移；direct `/backtrack --apply` 已复用 canonical replacement 与 composer/normal-send transaction；numeric `/rewind` 仍留在独立 checkpoint restore namespace）；
3. model/theme/skills 的 typed modal、fullscreen 或 mutation action，以及 numeric `/rewind` checkpoint restore；
4. 其余 slash command；
...
- **raw stdout 旁路**：P0 inventory 145 组/447 个 call site 直接写终端，绕过 surface 所有权——"下一帧覆盖不在 Scene 中的文字 / raw 输出出现在 ActiveBand 中间"的真源即此（§1.1、§1.4）。
...
补充的 Ctrl+T pager slice 已把 alternate renderer 的滚动 ownership 收敛到 actor：production pager loop 通过一次 `TranscriptPagerView` 读取当前 `ScreenLease` 对应的 `AppState.Transcript`、`Active` 与 `TranscriptOverlay.Pager`，避免跨 AppState revision 混用锚点和正文；按键仅投递带 `LeaseID` 的 `TranscriptPagerScroll`/`TranscriptPagerSetFollowBottom`，reducer 会拒绝旧 lease 的延迟 action。loop 比较 actor 已发布的 pager state，故 action 异步消费后无需再等输入或 transcript event 就会重绘；一旦 actor view 已配置但 action poster 不可用，循环保持只读，不能重新出现第二份本地 scroll state。打开 overlay 前先以 causal `ReplaceTranscriptAction` 将最新 Scene 发布到 actor，pager semantic snapshot 不再在 production 中混用 Scene 与 actor 的 transcript。无 actor 的 local state 只保留为 standalone/test fallback。该 slice 解决 Ctrl+T 的 view-state 双源缺口，但不改变 `FixedBottomSurface` 的 primary writer，也不代表 `TerminalSession`、history handoff 或 single-writer cutover 已完成。

direct supplement 与 direct assistant fallback producer 收敛的第一批实现现补齐 typed injection：没有 runtime event 的 local notice 通过 `EventEncoder.SubmitSupplement -> ChangeSetMapper -> Scene`，legacy non-stream final response 通过 `SubmitAssistant` 进入同一数据面；两种注入分别写 event log 并由 replay 重建。coordinator 的 `RenderLocalSupplement`/`RenderLocalAssistant` 只在 legacy cell 已写入且释放 `c.mu` 后发布 `ReplaceTranscriptAction`。输入拒绝、busy queue、ESC、goal/retry、provider retry、日志保存失败、direct invocation、live transcript supplement 与 legacy response fallback 已迁移，history replay 明确不注入。相反，runtime timeline/assistant 的 `RenderAsyncLine`/`RenderAssistant` 在 `Encode` 后只承担 legacy projection，测试固定其不得追加第二个 Scene cell。其后的 chat-core direct tool request/result 也已接入 typed data plane：`SubmitToolCall`/`SubmitToolResult` 以稳定 `tool_call_id` 维护唯一 mutable tool chain，结果输出合并后只 finalize 一个 `KindToolChain`；bridge 将等价 typed runtime event 追加进 event log，replay 因而重建相同 chain。历史工具回放通过 projection-only `RenderReplayedToolChainEvent`，不会第二次写 Scene；无 call ID 的 direct tool 仍留在 legacy projection，不推断身份。这是 producer data-plane 覆盖，不接 `TerminalSession`，不改变 ActiveBand/history handoff/`FixedBottomSurface`；approval transcript 与 raw system-output 仍在 inventory 中，不能把该迁移误报为全量收敛。

---

### 15.7 Primary Writer Cutover（2026-08-05）

interactive chat 的 primary 物理 writer 已从 legacy `FixedBottomSurface` 切到 `TerminalSessionPresenter`：启动时必须先 `EnableUnifiedRenderer` 建立 presenter，再 `SetSurface` 挂 compatibility facade，避免 surface 初始 status action 在 presenter attach 前抢写主终端；surface mount 仅可等待这一笔初始 actor barrier，流式输出与逐键输入不得等待 idle。`FlushEffect` 与 `HistoryCommitWakeEffect` 只能驱动 presenter 内的 `TerminalSessionExecutor`；`FixedBottomSurface` 仅保留 prompt/status/popup/legacy stream 的 compatibility facade state，`WriteOutput`、owned repaint、handoff 与 editor direct write 在 fence 下均不得发字节。`SetPrimaryPresenter` 拒绝 legacy writer 仍启用的会话，surface replacement 也不能解开 fence；shutdown 先 detach/drain presenter，再 close actor。

遗留 terminal control sequence 和 compatibility facade 的输出统一经过受锁的 `ui.TerminalOutput()` 代理，而不是异步 actor 直接读取 process-global `os.Stdout`。该代理只为 legacy adapter 的测试 capture 和 shutdown 顺序提供稳定 sink，**不**是第二个 production writer，也不得成为 unified interactive 的降级通道。

这满足 IR-2 的 primary-owner 切片，但不等于 P8 删除完成：active-stream/stable/soft/historyWindow 兼容状态、所有 semantic producer 的 AppState 覆盖、Ctrl+T pager 的端到端交互/PTY 验收与 raw-writer inventory 仍是后续工作。Ctrl+T 的 DEC 1049 transport 已通过 `ScreenLease` 委派给同一 presenter，不能 fallback 到 raw stdout；`TestChatInteractionCoordinatorUnifiedScreenLeaseUsesPresenterTransport` 进一步覆盖 coordinator 接线、overlay actor state、pager frame、lease release 与 source-backed primary recovery 的完整顺序。上文将 `FixedBottomSurface` 描述为“唯一 production writer”或将 `TerminalSessionExecutor` 描述为“未安装”的历史段落，以本节为准；不得据此放宽仍未完成的 P5-P9 验收条件。

---

### 15.8 Interactive Direct Cutover And Semantic Authority（2026-08-05）

interactive TTY 已直接选择 unified pipeline，不再支持 `AICLI_TUI=legacy/plain/off/0` 将同一会话退回旧 `FixedBottomSurface` renderer；JSON、`--no-interactive` 和非 TTY 仍是独立的 plain/JSON 生命周期，不是 interactive 的故障 fallback。若 unified presenter 无法 attach，session 初始化失败，不准启动一个 physical-write 已 fence 的 legacy surface。

切换后的 production 事实如下：

- `SetSemanticActiveCellProjectionAction` 是 writer attach 前的 actor barrier；开启后 reducer 清空并忽略 legacy `Bottom.ActiveBand*`，`LayoutAppScreen` 只从 `AppState.Active` 投影 mutable band。
- assistant delta/final 在 unified mode 只经过 `Scene -> UpdateActiveCellAction/FinalizeActiveCellAction -> AppState`；不再调用 `ActiveStreamController` 绘制、stable-commit queue、legacy ActiveBand 或 history-window 写入。final cell 以同一 Scene cell commit，不追加第二份正文。
- local assistant/supplement/error/command、MCP status、stable-id tool result 和 direct interactive text 均先进入 Scene；`printDirectInteractiveOutput`、`Surface.WriteOutput` 与 `fmt.Print` 的 legacy fallback 在 unified session fail-closed。persisted history 每次按稳定 identity 与已 replay Scene 一对一 reconcile，避免 partial event-log 漏历史或重复导入。
- `/help`、`/function(s)`、`/sessions`、`/new`、`/session`、`/history`、完整 `/debug`、`/stream`（含 `/s`、`/n`）、`/fast`、`/reasoning`、非选择器 `/reasoning_effort`、`/compact`、`/retry`、`/plan`、只读 `/timeline`、静态 `/collab`、静态 `/agents`、`/trust`/`/trust status`、`/queue`、`/attach`、非 `bypass_permissions` 的 `/permission-mode` 以及 `/approval-reuse` 已进入 `CommandResult -> Scene/AppState -> TerminalSessionPresenter`；这些有限命令不再因 primary cutover 被静默禁用。`/retry` 在 command cell commit 后通过 actor 输入状态恢复草稿，从不自动发送或覆盖现有输入；配置持久化、runtime session 同步或 debug archive 失败也作为同一 command cell 的 warning/错误，不得写 stderr；`/reasoning_effort select` 继续拒绝到 picker 迁移完成，`/collab follow` 继续拒绝到 typed effect/result stream 迁移完成，`/agents` 的 pick/send/target/panel/routing 继续拒绝到 typed input/effect 迁移完成，`/trust grant` 继续拒绝到 typed confirmation effect 迁移完成。仍未迁移的 modal、fullscreen、shell 流式命令由 unified command gate 生成语义拒绝 cell，禁止旧 handler raw stdout 回退；`!` shell shorthand、direct `/call`/`/skill` compatibility entrypoint 以及 coordinator teardown 后的 prompt/error/transcript fallback 同样 fail-closed，不允许绕过 `TerminalSession`。
- `ScreenLease` 的 DEC 1049 enter/write/exit、primary frame、history transaction 和 cursor 均由同一 `TerminalSession` serialize；lease release 强制从最新 AppState full repaint。

历史章节中“切换本身未开始”、“legacy surface 是唯一 production writer”或“`AICLI_SCENE_PRESENTER` 是 interactive production switch”的表述均只描述已退役的迁移阶段。它们不能覆盖本节和 15.7 的现状。尚未完成的是删除 compatibility-only state、审批/问答的 effect/result 化、异常流 finalization policy、全量 raw-writer debt 清零，以及 PTY/ConPTY 与长会话验收；这些工作不得恢复旧 writer。

---

### 15.9 异步 Runtime Event 会话身份边界（2026-08-05）

`chatRuntimeEventBridge` 的 worker 不得解引用可变的 `ChatSession.RuntimeSession` 来判断 event ownership。bridge 在创建时取得主会话 ID 的受锁快照，并且每次 runtime projection 被 restore、new conversation、sync 或 ephemeral plan session 替换时，通过 `updateChatRuntimeEventBridgePrimarySession` 显式发布新 ID。事件归属、异步 team summary、run-end fallback 与 prompt idle state lookup 都只能使用该快照。

这使 session restore/sync 与后台 event processor 有明确的同步边界：旧会话 event 在 ID 切换后被拒绝，新会话 event 被接收，且 worker 不会与 `RuntimeSession` 指针替换发生 data race。`TestChatRuntimeEventBridge_TracksPrimarySessionAcrossRuntimeProjectionReplacement` 固化该路由语义；本轮已通过 `go test -race ./cmd/aicli/commands -count=1` 和 `go test -race ./cmd/aicli/ui -count=1`。这只完成 commands/ui 范围的并发门禁，不替代 §19.4 的跨终端、长会话和全仓验收。
...
- [x] `go test -race ./cmd/aicli/commands -count=1` 与 `go test -race ./cmd/aicli/ui -count=1` 通过；
- [x] commands/ui 全量 Go tests 通过（race 命令覆盖完整 package 测试集）；
...
**切片 12 注记（bottom composer 输入 ingress：actor 背压不能冻结逐键编辑）**：已修复生产输入路径中 `InputBox` 每次 `OnChange` 经 `SetPromptInputSnapshot`/`RenderPromptInputSnapshot` 投递 `InputEvent` 后同步 `WaitIdle` 的阻塞链。该等待会把 editor goroutine 绑到 actor 正在处理的 runtime、terminal 或历史工作，actor mailbox 背压时表现为输入框完全不能键入。新路径先在独立 `promptInputMu` cache 内同步记录规范化 draft、cursor、paste state 并分配单调 `Sequence`，逐键路径不取得 coordinator 的 terminal/render `c.mu`，因此 editor 即刻读取到自身最新状态；带 Sequence 的完整快照 action 可 latest-wins 合并，旧队列 action 由 reducer 按 sequence 拒绝，`Sequence=0` 保留给 legacy/test durable FIFO。actor mailbox 已满时不调用阻塞 `Post`：coordinator 仅保留最后一个 deferred snapshot，以单一非阻塞 retry 在 mailbox 释放后投递；合并保留任一待处理的 `Render=true`，防止物理 prompt repaint 被后续 semantic-only update 吞掉。`ResetPromptState`、`DiscardPrompt` 和 shutdown 均推进 sequence 且清除对应 deferred snapshot，故 reset 前的 queued/deferred action 无法复活草稿。所有 terminal 写入仍在 actor reducer 的既有 `FixedBottomSurface` adapter 内发生，未引入第二个 primary writer，未连接 `TerminalSession`，也不改变 legacy history handoff。`TestChatInteractionCoordinatorPromptInputNeverWaitsForActorDrain`、`TestChatInteractionCoordinatorPromptInputNeverWaitsForCoordinatorRenderLock`、`TestChatInteractionCoordinatorPromptInputNeverWaitsForFullMailbox` 与 `TestChatInteractionCoordinatorPromptResetRejectsQueuedSnapshot` 覆盖 actor blocker、coordinator render lock、容量为 1 的 full mailbox 和 reset stale fence；目标 commands `-race` 已通过。

---

**切片 13 注记（chat-core direct tool chain）**：带稳定 `tool_call_id` 的 direct `tool_requested`/`tool_result` 已进入 EventEncoder/Scene typed path。`SubmitToolCall` 只创建 mutable chain，`SubmitToolResultDisplay` 以 `display_head` 将其一次 finalize；raw output/error 随等价 runtime event 落 event log 供诊断，live/replay 统一消费 display source。历史 tool 回放使用 projection-only adapter，no-duplicate 与 text-parity 回归已通过；无 call ID 仍保守 fallback。该数据面切片不等于 ActiveBand range owner、HistoryCommit、TerminalSession 或 single-writer cutover 完成，approval/raw system-output 仍在 inventory。

**切片 14 注记（TerminalSession transaction preflight gate）**：shadow `TerminalSession` 现在将 history handoff 与 viewport preflight 放在同一确认边界：HistoryCommit 虽已完成内存准备，若 frame 在 terminal write 之前验证失败，则 history 必须返回 `Deferred`，禁止以空 success 触发 Ack。该路径无 bytes 写出且不篡改既有 confirmed projection，executor 随后只能保留 token 并走有效 recovery/transaction。该 hard gate 仅覆盖非生产 seam；`FixedBottomSurface` 仍是唯一 production writer，lease transport 和 single-writer cutover 未发生。

**切片 15 注记（semantic boundary change and history payload rebase）**：history planner 现在区分“同一 semantic source 的当前 display payload”与 token identity。前置 cell 的 boundary/gap 变化会使 pending commit 原 token rebase 到新的 display range/render lines；已进入 in-flight 的旧 payload 则 fail-closed invalidation 并要求 projection recovery，避免旧 Ack 造成错位或重复 native scrollback。该切片只强化 reducer-owned effect ledger/data-plane，不改变 `FixedBottomSurface` 的 production writer 角色。

**切片 16 注记（priority transcript and raw tool-output ownership）**：同步 approval/question 例外仍未拆为 reducer interaction effect，但其 semantic transcript 已收敛：request event 只登记待交互 identity，不占用 RenderModel/Scene 的可见顺序；输入完成时才以 request key 追加一个 completed `KindPriorityPrompt`。因此 permission/reuse hint 会先于最终 transcript 出现在 legacy history、Scene 与 AppState；resolved/answered 即使早于 transcript 到达也只记录生命周期，不能丢弃晚到 transcript，重复 request/resolution/replay 均不会附加第二个 item。没有 runtime event source 的 permission/reuse/decision 则单独经 `RenderLocalSupplement` typed data plane 记录，已 Encode 事件不得再从 `RenderAsyncLine` 反向注入。stable `tool_call_id` 的 direct raw shell output 和上限提示只投影到 ActiveBand，最终 `tool_result` 是唯一 durable chain；actor Execute/Continue 移除了全局 retained OutputMirror，改由已有 stable `tool.progress` 负责 live display。该切片消除两个主要 duplicate writer，但仍未迁移 ActiveBand range owner、HistoryCommit executor、TerminalSession transport 或 Presenter single-writer cutover；`FixedBottomSurface` 继续是唯一 production physical terminal writer。
