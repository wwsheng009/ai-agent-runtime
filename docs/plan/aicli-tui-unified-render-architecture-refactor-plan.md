...
interactive chat 的 primary 物理 writer 已从 legacy `FixedBottomSurface` 切到 `TerminalSessionPresenter`：启动时先 fence legacy physical write，再在 UI actor 创建后用 late-bound effect consumer attach presenter。`FlushEffect` 与 `HistoryCommitWakeEffect` 只能驱动 presenter 内的 `TerminalSessionExecutor`；`FixedBottomSurface` 仅保留 prompt/status/popup/legacy stream 的 compatibility facade state，`WriteOutput`、owned repaint、handoff 与 editor direct write 在 fence 下均不得发字节。`SetPrimaryPresenter` 拒绝 legacy writer 仍启用的会话，surface replacement 也不能解开 fence；shutdown 先 detach/drain presenter，再 close actor。
