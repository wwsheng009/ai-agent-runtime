...
> **2026-08-05 cutover 更正**：上方 Phase 2、history-effect 与 `TerminalSession seam` 三段均是 pre-cutover 的历史增量，不能再用于判断当前 primary。现在 owned interactive 的 `TerminalSessionPresenter` 已消费同一 `AppState -> Layout/Compose` frame，并在同一 terminal transaction 内执行 `HistoryCommit` handoff、viewport diff 和 cursor；`FixedBottomSurface` 的 physical writer 与 legacy history handoff 已 fence。尚未完成的是 AppState 的全 producer/interaction 覆盖、compatibility state 删除、Markdown/超长单行的 source fragment handoff 与真实 PTY/ConPTY 验收。
