...
> **衔接（2026-08-02）**：本小节管的是"事件 → 重绘请求"的**调度面**；内容在屏幕上的**位置与身份**不由本层决定。LLM/工具事件应先经统一编码器（`EventEncoder.Encode` → `ChangeSet`，见 [统一渲染编码器方案](./aicli-event-stream-rendering-order-unified-encoder-plan.md)），本层以编码器输出的有序模型/变更集为输入请求重绘——事件循环不直接消费原始 delta 决定渲染位置。
