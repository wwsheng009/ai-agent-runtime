...
- **Backend HTTP**: `internal/api/skills/handler.go` 只暴露通用的 `POST /runtime/fs/read-file`（base64），没有专门、按 session scoped 的图片端点。直接复用 `fs/read-file` 会让 UI 拿到“任意路径读权限”，安全性不足。
- **Frontend 类型与渲染**:
...
│ Codex 响应处理        │ -> │ GET /runtime/sessions/{id}/ │ -> │ image segment + image│
...
```
GET /api/skills/runtime/sessions/{id}/generated-images/{name}
```
...
- `src` 拼接规则:
  ```ts
  `${runtimeBaseUrl}/api/skills/runtime/sessions/${encodeURIComponent(sessionId)}/generated-images/${encodeURIComponent(sanitizedId || basename(savedPath))}`
  ```
...
- runtime-server 现有 `/runtime/fs/read-file` 不动，依然可作为运维兜底。
