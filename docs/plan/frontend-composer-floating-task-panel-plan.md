...
状态：**草案（待评审）**——本轮只做「现状取证 + 参考实现分析 + 实施方案」，不含任何代码改动。
...
| 是否有相近先例 | `pending-interaction-bar.tsx`、`jobs-status-bar.tsx`、`session-goal-indicator.tsx`、`artifact-panel-plan-surface.tsx` | 分别覆盖审批/提问、后台作业计数、会话目标、plan-mode 评审——**没有一个覆盖 agent 的 todo 列表** |
...
5. 复用当前项目的既有约定（Tailwind 语义 token、i18n 双语资源、Vitest + testing-library、行数门禁 ≤ 500 行），不引入新依赖、不引入插件/插槽框架。
...
> 移植结论：当前项目的 composer **已经是** `absolute inset-x-0 bottom-0 z-30` 的吸底 overlay 容器（`main-section.tsx:418-446`），
...
| 语义差异（vs 参考实现） | 本项目管理更严：**同一时刻只允许一个 `in_progress`**；多传会「软修复」——保留最后一项、其余降级为 `pending`，并在结果里回报 `multi_in_progress_healed` / `demoted_in_progress` | `toolkit/tools/todos.go:175-178, 251-275` |
...
| `todos` 工具行今天等于「无信息通用行」：工具名未注册（落到 `generic` 扳手图标），且摘要读取器**不读** `summary`/`summary_lines` | `lib/tool-row/kind.ts:13-85`、`lib/thread-state/tools.ts:70-92`、`components/workspace/message-tool-row.tsx:166-173` |
| 前端运行时事件缓冲是最近 **100** 条滚动窗口（面板数据不能只靠它） | `lib/thread-state/history-artifacts.ts:15`、`lib/thread-state/events.ts:387` |
| i18n 资源按域分文件，workspace 域已有 12 个词典文件（`panels-*.ts` + `base.ts`） | `frontend/src/i18n/resources/{zh-CN,en-US}/workspace/` |
...
| F7 | 历史原文 `session-history-{id}` 逐字保留，但内部上下文消息（含 `todo_state`）被过滤掉；工具消息 metadata 是否幸存**未验证** | `lib/thread-state/history-artifacts.ts:151-158, 190-192, 220-237` | 决定 B1 是否可行的唯一未知量，方案把它列为**先探针后实现** |
...
- 命名用 `todo_snapshot`（或 `todos`），**不要改名生产侧已有的 `metadata["todos"]`**——`compact_reconciliation.go:80-101` 正读这个键，改名会连带改 F2 的既有实现和测试。
- 结果：`tool.completed` 在 live 与「同页会话内重放」两种情况下都能自给自足，前端不再依赖文本摘要。

**通道 B —— 冷启动/刷新后的权威快照（二选一，先探针）**

- **B1（首选，前端零后端改动）**：从已逐字保留的 `session-history-{id}` 原文中取 `metadata.todos`。
  - **探针（实现前必做，30 分钟内可判定）**：打开一个调用过 `todos` 的会话 → 在 devtools 里查看该会话的 `session-history-{id}` 产物 JSON → 搜索 `"todos"` / `"metadata"`。
  - 判定：命中 → 直接派生（刷新后仍有列表，零后端成本）；未命中 → 转 B2。
...
- **可见性**：
  ...
  - 会话切换 → 视为新快照，折叠态按会话维度记在 `sessionStorage`（与 P2-9 顶部作业条同一套「仅 live>0 渲染」的克制策略）。
...
## 7. 任务拆解（建议按 T0 → T1 → T2 → T3 → T4 顺序，T1/T2 可并行）

| ID | 任务 | 交付物 | 文件（新增 / 修改） | 依赖 |
|---|---|---|---|---|
| **T0** | **B1 探针**（0.5h，先做，决定 B 通道选型） | 一句话结论 + devtools JSON 证据片段（是否含 `metadata.todos`） | 无代码改动 | — |
...
| **T2b** | 仅当 T0 判定 B1 不可用时：B2 只读接口 | 后端 handler + 前端 api + 测试 | 后端仿 `api/skills/handler.go:778-779`、复用 `chat/compact_reconciliation.go:76-101`；前端改 `api/runtime/sessions.ts`（仿 `:361-389`） | T0 |
...
**范围控制**：T1 只改「放行」，不改生产侧 metadata 键名与 `compact_reconciliation` 的读取契约；T3 只加一个 DOM 节点，不动 composer 内部结构与 `pointer-events` 语义。
...
| 构建/e2e | `npm run build` → `npm run test:e2e` | **必须先 build 再跑 e2e**（e2e 只吃 `dist/` 产物，仓库已有一次假失败教训） |
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| B1 探针失败（历史原文不含工具 metadata） | 刷新后无列表 | 转 B2 只读接口（T2b）；排期增加约一个后端 handler + 测试 |
...
> 因此 B1 不是「押运气」，而是**复用既有契约**。建议把 T0 从「30 分钟 devtools 手工探针」降级为「一条断言」——`curl .../sessions/{id}/history | jq '.. | .metadata?.todos? // empty'`，或直接由 T2 的单测夹具覆盖。**唯一尚未逐行追证的一环**是第 1 步中 `gateway.Process` 的入参 metadata 是否逐键等于工具结果 Metadata（本审查已定位到写入点，未回溯到 gateway 内部）；保留 5 分钟确认即可，不影响选型。
...
| D2 | §6.3 | 折叠态用 `sessionStorage` 持久化——`frontend/src` 全量搜索 **0 命中**，仓库无先例；面板在同会话内是常驻挂载，本身无跨挂载持久化需求 | 改为**组件内 state**（与 `PendingInteractionBar`、goal 指示器一致）；§8.2 第 6 条「会话内保持」用本地 state 即可满足 |
...
| 实施可执行性 | **基本完整**：T0–T4 有文件级落点、有依赖、有验收；缺 T5 文档任务（D5）与两处定稿（D1/D2） |
| 测试与验收 | 完整（后端新增+回归、前端单测、手工清单九条）；e2e 处置待明确（D3） |
| 风险与决策 | 完整（§9 三张表 + 3 个开放问题）；缺兼容性结论（D4） |

**总评：无 blocker 级缺失，完整性足以支撑评审**。建议按「§10.2 已修 3 处矛盾 + §10.3 收口 6 项（D1/D2/D3 为定稿，D4/D5/D6 为补文）」的顺序推进，再开 T1。

### 10.6 锚点逐条复核（独立子代理 · 结果待回填）

已委派独立只读子代理对本文档 22 处高频引用锚点（`todos.go` / `tool_runtime_events.go` / `result.go` / `compact_reconciliation.go` / `kind.ts` / `tools.ts` / `use-session-runtime-stream.ts` / `main-section.tsx` 等）做行号级复核，判定口径为 `OK / 偏移 / 不匹配 / 文件不存在`。结果与偏差清单在本节回填；**回填前，§4/§5 的锚点行号按「作者本轮实测」采信**。
