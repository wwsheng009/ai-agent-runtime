...
| C3 | P2-7 命令系统无内置执行器 | 部分交付（第 3 子片已交付） | 中 | 已登记；**内置命令清单与执行器（`/export` / `/rename`）已交付并回填（批次 12）；`/model` 目录弹窗与 `/feedback` log-only 入口已交付并回填（批次 13）**，余命令专属 popupSelect 候选待排期 |
