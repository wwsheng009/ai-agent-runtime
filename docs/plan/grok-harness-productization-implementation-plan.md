...
更新时间: 2026-07-26  
状态: Iteration A 完成（含 A6 Agent Source residual）；**Iteration B 核心已收口**（B1–B6，含 general.md）；**Iteration C 核心已收口**（C1–C3 + C4a doom-loop + C4b tool search + C4c optional OS sandbox）；**R1–R7 residual 已收口**；**R3 pure-advisory strip-on-persist residual 已收口**；旁路 **backtrack frontend residual 已收口**；旁路 **STALE_CONTEXT pipeline residual 已收口**（含 2026-07-26 导出 refine + edit whitespace/column/blank-run auto-heal + **edit/apply_patch current_snippet copy-paste recovery** + **model contract 导出 current_snippet 文本** + **edit/apply_patch multi-line closest near-identifier typo ranking** + **true content_diff token fallback / 截断 %q rehydrate**）；旁路 **path_missing auto-heal + candidate quality residual 已收口**（唯一高置信 sibling 改写 + `.backups` 噪声过滤 + parent discovery）；旁路 **apply_patch invalid syntax / path crumb residual 已收口**（2026-07-26：`TOOL_INVALID_ARGS` + `sanitizeApplyPatchPath`）；下一波仅 R8+ 可选产品线或其它明确 ROI 项  
...
| 2026-07-26 | STALE_CONTEXT pipeline residual 收口：`recordToolExecutionOutcome` 保留 tool-authored code + 提升 disposition；`Diagnose` 认 nested next_action；`tool.completed` 导出 `failure_class`/`file_path`/`suggested_view_*`；见 `docs/analysis/aicli-session-optimization-plan.md` P1-1 |
| 2026-07-26 | STALE_CONTEXT 导出 refine residual：`Diagnose` 将 generic `TOOL_EXECUTION` + edit/hunk miss / `failure_class=stale_context` refine 为 `STALE_CONTEXT`（替换 generic next_action）；live/offline efficiency 映射 `STALE_CONTEXT`/`SPAWN_DEPTH_LIMIT`；manager meta + payload 测 green |
| 2026-07-26 | STALE recovery residual：edit indent/whitespace auto-heal + 多行 closest 块；`current_snippet`/`current_snippet_start_line` metadata + promote/export；model contract 暴露 `suggested_view_*`/`file_path`；disposition advisory 引导复制 snippet；`go test` toolkit/toolresult/output/agent green |
| 2026-07-26 | apply_patch STALE recovery residual：`patchHunkNotFoundError` 结构化携带 closest；metadata 导出 `current_snippet`/`current_snippet_start_line`（与 edit 对齐）；`next_action` 优先复制 snippet；`go test ./internal/toolkit/tools ./internal/toolresult ./internal/agent` green |
| 2026-07-26 | path_missing residual 收口：read-safe unique sibling auto-heal（`path_auto_healed`）+ candidate quality（过滤 `.backups`/VCS 噪声、无 sibling 时 parent discovery、PATH next_action/disposition advisory）；`go test ./internal/toolexec ./internal/toolresult ./internal/agent` green |
| 2026-07-26 | path placeholder empty residual：`path=""`/`null` 占位 token 在 preflight 归一为真 empty，避免 workspace 根目录噪声 candidates；可选 path Allow、必填 path → `TOOL_INVALID_ARGS`；`go test ./internal/toolexec` green |
| 2026-07-26 | edit STALE content_diff residual：column-ws collapse + blank-run flexible match + `rebuildEditReplacement` 保文件字节；model contract 导出 `current_snippet`（4KiB 整行 cap、不 TrimSpace）；`go test` toolkit/toolresult/output/agent/tools green；见 optimization-plan P1-1 |
| 2026-07-26 | dashboard STALE snippet rehydrate + path ambiguity residual：`tool_efficiency_common.rehydrate_tool_result_fields` 从 body 回填 `current_snippet`（header 形 + numbered-line 收紧，拒 prose/`期望内容` 假 snip）；歧义 path next_action Pick exactly one；`go test ./internal/toolresult ./internal/toolexec` + `python -m unittest tmp.test_tool_efficiency_common` green |
| 2026-07-26 | STALE closest near-identifier residual：`edit`/`apply_patch` multi-line closest 对 `HelloWord`↔`HelloWorld` 等 near-typo 用 `runeEditSimilarity` 软匹配；generic structural 行降权；apply_patch 用 winning expected 子集 tighten + 右偏 pad，避免 late anchor 左扩到 import 噪声；单测 `TestFindClosestEditSnippetWithLine_AnchorsOnLaterDistinctiveLine` / `TestClosestPatchCurrentContext_AnchorsOnNearIdentifierTypo` green |
