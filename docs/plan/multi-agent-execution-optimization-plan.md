...
更新时间: 2026-09-13
状态: implementing（P0-1/P0-2/P0-4/P1-6/P1-7/P2-8/P2-9 已实施；P0-3/P2-11/P2-12 已实施（P0-3 三条补偿路径 actor/prompt/snapshot 在 API 与 CLI 两宿主同口径，故障注入与配额不变量用例齐备；P2-11 工具描述层已同步；P2-12 宣告层宿主能力过滤 + CLI `/debug supervision control` 已落地，见 §5 实施注记与 §10 E4），其中 P2-9 手动清理入口 `/agents cleanup` 与「终态/空闲自动 close」（周期对账 observe 只报候选、enforce 才落库）已落地（TTL 删除按方案建议走对账兜底、不做直接回调）；P0-4 仅剩真实终端 probe 与 production-readiness 文档差异标注；P1-6 wake 预算已补齐三处同源同序的可见性（`/supervision/digest` 与 `/supervision/snapshot` 的 `wake_budget` 投影、CLI `/debug supervision list`、以及模型可见的 preflight digest 文本行；父 turn 结束自检（方案 4）已实施并默认关闭，需显式开 `supervision.wake_self_check_per_window`）；P2-8 产品事件流 `agent.reclaimed` 已接前端轨迹（仅剩真机长跑 probe）；P1-7 最后一项待补已补齐（相同 `after_seq` 的重复读返回 `unchanged` / `repeat_count` 显式信号，不再只靠 `next_action` 间接引导）；P1-5 四个方案全部实施——前端下钻、`read_agent_events` 过滤视图（`view=tool_progress`）、父流节流镜像（`subagent.progress` 后端镜像 + 前端 `runtime-0` 折叠行）与本轮补齐的**方案 4「前端 inline 审批」**（下钻对话框内直接批准/拒绝，动作走 actor `approve_tool` 命令），仅剩真机 probe；P2-10 单测与并发压测已补（真实终端 probe 未做）。实施记录见 §10）
...
**实施注记（2026-09-13 复核）**：补偿已落地，覆盖 `GetOrCreate` 与 `SubmitPromptAsync` 失败（API `backend/internal/api/skills/session_runtime_support.go:589-613`、CLI `backend/cmd/aicli/commands/chat_actor_registry.go:705-729`），`ReleaseAgentControlAgentSpawn`（`backend/internal/agentcontrol/agent_registry.go:249-258`）与 API 侧 stale 预检也已下沉；**`snapshot` 读回失败分支（口径已定，2026-09-15 收口）**：按 `queued` 分流——没有排队任何 prompt 时（不存在会把这笔预约推向终态的 run）走赔偿 `rollbackSpawnFailure(fmt.Errorf("snapshot child session: %w", err))`；已排队时只上报诊断原因、不删除子会话（删一个活着的 agent 比留一条待收敛的行更糟），由完成回调收敛预约。两宿主同口径、同措辞（API `session_runtime_support.go`、CLI `chat_actor_registry.go`）。验收侧已补：API 级故障注入 `backend/internal/api/skills/session_agent_spawn_rollback_test.go`（注入 `GetOrCreate` 失败，断言 stale 终态 + 半成品会话删除 + active 只剩 root）、store 级 release 幂等（`global_agent_store_test.go:445-465`）与并发守恒（`concurrency_stress_test.go`）；**已补**：`SubmitPromptAsync` 失败注入（API 级，注入手法为「创建即 Stop」的 actor）、CLI 同场景用例（快照失败回滚 / 已排队不回滚 / 名额已满不留行）与配额不变量（store 级并发守恒已有，宿主级「上限拒绝不留行」本轮补齐）。本条按“已实施”计，细节见 §10 E4。
...
| P2-11 协作引导（prompt 层 + 工具描述层） | 已实施（两层同源：协作纪律与等待边界常量落在 `backend/internal/agentguidance`，提示词与工具描述都引用同一份；`wait_agent`/`read_agent_events`/`wait_team` 三条等待路径共用同一 `agentcontrol.ResolveWaitTimeout` 与宿主策略，见附录 E2） | 渲染器 `backend/internal/prompt/environment_context.go:236-254`（`RenderMultiAgentCollaborationGuidance`，段落头 `Multi-agent collaboration guidance:`，7 条纪律）；API 宿主注入 `backend/internal/api/skills/instruction_messages.go:118-166`（`withDelegationGuidance:127` = difficulty + collaboration，`withInstructionGuidance:143` 幂等判定，接线 `buildRuntimeInstructionMessages:11-18`，覆盖 handler/子代理/调试三处调用）；CLI 宿主注入 `backend/cmd/aicli/commands/chat_session.go:1590-1592`（`composeDurableChatSystemPromptWithGuidanceForCWD`） | `go test ./internal/prompt -run "MultiAgentCollaboration" -count=1`；`go test ./internal/api/skills -run "InstructionMessages" -count=1`；`go test ./cmd/aicli/commands -run "TestComposeLocalChatSystemPrompt_IncludesWorkspaceGuidance" -count=1`；`go test ./internal/toolbroker -run "Wait" -count=1`（wait_agent/read_agent_events/wait_team 边界与两面守卫）；`go test ./internal/api/skills -run "StampsBrokerWaitTimeoutPolicy" -count=1`；`go test ./cmd/aicli/commands -run "StampsSharedWaitTimeoutPolicyOnBroker" -count=1` |
...
- P2-11 协作引导：**部分实施**（`internal/prompt` 新增 `RenderMultiAgentCollaborationGuidance`，API 与 CLI 两宿主注入 system instruction；工具描述层已与提示词同源（见附录 E2）：共享纪律常量落在 `internal/agentguidance`，两个 `wait_agent` 变体的 `timeout_ms`、两个 `read_agent_events` 变体的 `wait_ms` 与 `wait_team` 的 `timeout_ms` 都渲染运行时真实边界与 clamp 回显，三条等待路径共用同一 `agentcontrol.ResolveWaitTimeout`（宿主策略经 `Broker.WaitTimeoutPolicy` 惰性注入，与各自 `wait_agent`/`read_agent_events` 用的是同一个 mapper），并有运行期+源码双向守卫与两宿主接线守卫防止只改一个变体）。**未做**的是 ① 真机验证引导对模型行为的实际影响（spawn 后是否先做独立工作、是否减少同窗口重读）② `internal/agent` 子代理 prompt 的协作段落（默认 `maxDepth=1` 时子代理无 spawn 能力，暂不注入）。
- P2-12 监督通知闭环（本地宿主）：**部分实施**——CLI `list|ack|defer|resolve`、模型工具 `supervision_snapshot` / `ack_lifecycle` / `control_descendant` 与 stale 判定已落地；① 的 CLI 入口已落地：`/debug supervision control <id> --action … --reason … [--cascade target|descendants]` 复用同一 durable 动作服务与 CAS（见附录 E1）。**未做**的是 ② 方案 1 的「动作集合按宿主能力过滤」（`Evaluator.EvaluateAllowedActions` 仍只接收 `Notification`，当前只按 stale/存在性降级，尚未按宿主可执行集合裁剪，见 §5 P2-12 实施注记）。
...
- **等待预算语义（2026-09-13 补强）**：实测中模型对 `polling_backoff` 提示的响应是**加大 `timeout_ms`**（5m → 7m → 10m），而原指纹把 `timeout_ms` 计入参数摘要，导致每次加大都被当成"新请求"、`repeat_count` 归 1，提示不再触发 —— 父 turn 可在零通知下冻结数十分钟。现改为：①指纹剔除纯调度参数（`pollingTimingOnlyArgs` / `pollingFingerprintArgs`，覆盖 `timeout`/`timeout_ms`/`wait_ms`/`poll_ms` 等；观察目标 ids/after_seq 仍参与指纹）；②新增**累计等待预算**：`PollingWaitBudgetNoticeThreshold`（5m）与 `cumulativeWait`（单次上限 `maxPollingWaitMillis`=24h），重复次数或累计等待任一命中即注入提示，`budgetNotified`/`noticeEmitted` 保证每段连续只发一次，`pollingWaitBudgetAdvisory()` 以累计时长为主语；③`polling_backoff` 事件 payload 增加 `cumulative_wait_ms` / `wait_budget_ms` / `wait_budget_exceeded`（`backend/internal/agent/loop.go:876-890`），`PollingBackoffObservation` 同步 `CumulativeWait`/`WaitBudgetExceeded`；④真实工作批次仍 `reset()`（含清零累计等待）。测试：`polling_guard_test.go` 的 `TestPollingBackoffTracker_TimingArgsDoNotBreakStreak`（timeout 变化不断链、换 target id 仍断链）与 `TestPollingBackoffTracker_EscalatingTimeoutsStillTripWaitBudget`（threshold=5、5m→7m→10m 累计 22m、只发一次 notice、advisory 含 `22m0s`）。
...
- **2026-09-23 工具面收敛（命名口径更新）**：模型可见的监督工具由 7 个收敛为 4 个，统一改用 `subagent_*` 前缀让名字自解释：`subagent_status`（台账 rollup；`include_digest=true` 即原 `supervision_snapshot` 的摘要）、`subagent_inspect_task`（单标的深查；`include_status=false` 即原 `read_agent_result`）、`subagent_ack_lifecycle`（原 `ack_lifecycle`）、`subagent_control`（原 `control_descendant`）。三个旧定义名 `supervision_snapshot` / `supervision_descendants` / `read_agent_result` 降级为「可派发、不广告」（`internal/toolbroker/supervision_tools.go` 的 `retiredSupervisionToolNames` + `advertisedSupervisionToolDefinitions` 过滤），两个旧别名拼写 `ack_lifecycle` / `control_descendant` 由 `normalizeToolName` 归一到新名；`Definitions()` 只暴露新名，已存提示词、历史指针与旧调用不断。`read_agent_result` 同时补进工具策略/只读分类表（`internal/policy/tool_policy.go`、`internal/policy/taxonomy.go`、`internal/policy/capability.go`、`internal/toolkit/listable.go`），避免窄白名单静默丢弃定义。同轮审计未发现其它重复定义（`get_goal`/`read_goal`、`spawn_subagents` 等只是别名拼写，无独立定义项）。
...
- **observe / enforce 的差别只在是否写库**：observe（默认，含 `AICLI_REGISTRY_RECONCILE_MODE=observe`）只调 `EvaluateReclaimOutcome`，报告 `reclaim_candidates=<n> reclaim_reasons=<去重原因>`，不关闭任何行、不产生产品事件；enforce 才真正关闭，报告 `reclaimed=<n> reclaimed_rows=<行数> reclaim_reasons=…`（reasons 取自实际执行的决策），单个对象失败只累计 `reclaim_failed` / `reclaim_error` 而不断整轮。两模式共用 `AppendReclaimReason` 去重：`EvaluateReclaimOutcome` 与 `ReclaimAgentQuota` 都把折叠后的原因写回 `outcome.Reasons`（后者原先只追加 `Decisions`、不回填 reasons，`Summary` 兜底逐条拼接，才渲染出 `reclaim_reasons=session_terminal,session_terminal,session_terminal`），`ReclaimOutcome.Summary`（闸门 extras、`/agents cleanup` 输出、`agent.reclaimed` 的 `summary` 字段三个出口的唯一汇聚点）再折叠一次，因此一行摘要不会随触发者或子会话数变长。
...
- **人读面与机器面分层（避免 `reclaimed=3 reclaim_rows=3 reclaim_reasons=session_terminal` 污染终端）**：`ReclaimOutcome.HumanSummary()`（`backend/internal/agentcontrol/reclaim.go`）是 `/agents cleanup` 的人读出口，一句话回答「发生了什么 / 释放了多少槽位 / 是否需要关注」，例如 `已自动回收 3 个已结束的子会话；释放 3 个线程槽位`，只有失败才追加 `N 个未能回收：<error>（需要关注）`（observe 模式说 `可回收 …（仅评估，未执行回收）`）；同一 pass 的计数原样保留在紧随其后的 `详情: reclaimed=… reclaim_reasons=…` 行，闸门 extras、`/debug`、`agent.reclaimed` payload 的 `summary` 字段继续只用 `Summary()`，机器契约不变。原因词表收在 `ReclaimReasonLabel`（`session_terminal`→已结束的子会话、`session_missing`→已不存在的子会话、`idle_timeout`→长期空闲的子会话），未收录的 reason 以 `reason=<原值>` 透出而不被泛化标签吞掉。CLI 时间线对产品事件 `agent.reclaimed` 走 `HumanReclaimSummaryForPayload`（`backend/internal/agentcontrol/reclaim_events.go`），渲染为 `[subagents]` 提示行并以来源开头（`spawn 配额回收` / `手动清理` / `周期对账`），因此「谁触发的驱逐」无需再读 payload；前端既有英文行 `agent reclaimed: …` 不在本次改动内，词表以其为准再对齐。

- **终态保留与 GC（retention，已实施）**：终态行（`closed` / `stale`）与它们产生的 wake 事件不再永久堆积——`agentcontrol.PurgeTerminalAgentRecords`（`backend/internal/agentcontrol/retention.go`）接在每次对账 pass 的审计与驱逐之后，按 `closed_at` / `created_at` 严格早于窗口的条件分批删除（单批 `terminalPurgeBatch`，单 pass 至多 `terminalPurgeMaxBatches` 批，避免在共享 SQLite 上持长写事务）。**行的一半严格只命中「已经终态且早于窗口」的对象**，所以 purge 不会与仍持配额的子会话竞争；wake 事件的一半按 `created_at` 覆盖整条 append-only 日志（每次身份变化都会追加事件、无变化的投影刷新不会，见下条；活跃行不设上限同样会无界增长），因此从早于窗口的 `after_seq` 续读可能看到历史已过期的事件——窗口就是这份日志的保留契约；store 不支持时报告 `Supported=false` 而不报错（钩子可无条件装配）。窗口口径两宿主一致：`agents.registryTerminalRetention`（`RuntimeConfig.Agents`，Go duration，默认 30 天 = `agentcontrol.DefaultTerminalRetention`）与环境变量 `AICLI_REGISTRY_RETENTION` 的优先级为 env > runtime config > 内置默认；`off` / `none` / `-1`（或负配置值）归一化为 0 = **关闭 GC**（保留全部终态行供诊断），非法值静默回退到下一优先级（`ParseTerminalRetention` → `NormalizeTerminalRetention`），不会因为一次手滑把 GC 关掉。结果并入同一份对账摘要：`purged_rows=N purged_wake_events=M`，失败以 `purge_error=…` 记录而不清空 drift 报告（`ReconcileReport.Purged*` / `Reconciler.Purge`）；CLI 与 API 两个宿主的 `Purge` 钩子指向同一策略，因此同一份 registry 在两宿主下收敛节奏一致。
- **宿主投影一致性（G3/G4/G5，已实施）**：API 宿主的 `materializeAgentControlAgentProjections` 现在与 CLI 的 `materializeLocalAgentRegistry` 同口径——刷新投影前先 `sweepStaleAgentControlAgentRegistry` 收敛「容器已消失 / 已终态」的 active 行，再写投影（此前 API 只在审计里报告漂移、不收敛，导致同一份 registry 在两个宿主下的 active 集合不同）；无法解析会话绑定的宿主（无 session storage / runtime state store）不收敛，避免把「无法验证」当成「已失联」。物化全程单飞（`agentControlMaterializeMu`），一次 O(sessions) 扫描不会被 spawn 门控、列表刷新与周期对账叠加；本次同时以 `agentControlRecordIndex`（一次列表建内存索引）取代每条投影记录一次点查（N+1 → 1）。
- **无变化刷新不再追加 wake 事件（写放大收敛，已实施）**：宿主的投影物化每次 pass 都会重写全部行（`projectLocalAgentRecords` 列出全部 session 后逐条 upsert），而 `UpsertAgentControlAgent` 此前在每次写入后无条件 append 一条 `upsert` wake 事件并通知 watcher——于是「10 分钟对账 + 每次 spawn 门控/列表刷新」× 每行 = 与「变更」无关的事件洪水（retention GC 是兜底而不是解法）。现在 store 先取旧行、写入后比对持久化形态（身份/路径/深度/路由/状态/`closed_at`/`route_warnings`，见 `agentRecordsShareWakeState`）：**只推进 `updated_at` 的刷新不再追加事件、不再通知 watcher**；任何真实字段变化（含新建行）仍照常发事件。选「保留写入、只抑制事件」而不是「跳过写入」，是因为 `ReclaimObservation.RegistryUpdatedAt` 是空闲判定的兜底输入（`reclaim.go` 的 `idleSince`），心跳必须继续推进。顺带修正 `AgentWakeEvent.Seq` 的注释——它是 wake 日志自己的序列（`agent_control_agent_wake_events.id`），与 `AgentRecord.Seq`（行号）是两套 id 空间：续读 wake 用 `LastAgentControlAgentWakeSeq`，续读行用 `ListAgentControlAgents(AfterSeq)`。
- **验证（2026-09-14）**：`go build ./...` exit 0；`go vet ./internal/agentcontrol/...` exit 0；`go test ./internal/agentcontrol/... -count=1` ok 3.424s（新增 `TestUpsertAgentControlAgentRefreshKeepsHeartbeatWithoutWakeEvent`：新行发事件 → 同内容刷新不再发事件且 `updated_at` 前进 → 改 `EffectiveModel`/`RouteWarnings` 照常发事件；`TestAgentRecordsShareWakeStateTracksPersistedShape` 8 个子用例固定比对口径，含「nil 与空 `route_warnings` 等价」「零值 `closed_at` 视同 NULL」）；`go test ./internal/api/skills/... -count=1` ok 16.521s；`go test ./cmd/aicli/commands/... -count=1` ok 79.927s，其中新增 `TestLocalRegistryMaterialize_UnchangedPassDoesNotAppendWakeEvents` 在连续 3 次物化 pass 后断言 `LastAgentControlAgentWakeSeq` 不变（端到端证明投影刷新不再刷事件）。

- **retention 查询计划（已实施，2026-09-14）**：两处 purge 语句此前把时间列包在 `datetime()` 里（`ORDER BY datetime(closed_at) ASC, id ASC`），`EXPLAIN QUERY PLAN` 显示每批都退化成 `USE TEMP B-TREE FOR ORDER BY`——`idx_agent_control_agents_closed_at` / `idx_agent_control_agent_wake_created_at` 正是为「按年龄分批删除」建的，而表达式排序无法由索引满足，于是万级终端行下每批都要排序全部命中行。现在范围条件与排序都改成裸列比较（`closed_at < ?`、`ORDER BY closed_at ASC, id ASC`），并保留 `datetime(column) < datetime(?)` 作为年龄的权威判定：SQLite 因此走覆盖索引有序扫描 + `LIMIT` 提前停止，同时「无法解析时间戳的行不删」的保守语义不变。两处语句提为包级常量（`purgeTerminalAgentsSQL` / `purgeAgentWakeEventsSQL`），让 `retention_plan_test.go` 对**真正执行的 SQL** 跑计划守卫（断言出现 `SEARCH … USING` 且命名到 `idx_agent_control_agents_closed_at` / `idx_agent_control_agent_wake_created_at`、不出现 `TEMP B-TREE`；仅断言「有 SEARCH」不够——废弃 `datetime()` 但去掉范围条件时规划器可能仍选到别的索引有序扫描），而不是对测试里的副本断言。边界：`closed_at IS NULL` 恒不命中；`RFC3339Nano` 会省略尾随零，同一秒内 `…:00Z` 的字典序排在 `…:00.5Z` 之后，最多让某一批挑走一行误差 ≤1 秒的对象，且 `datetime()` 判定仍会剔除不符合年龄条件的行——「窗口最终删掉哪些行」不变。
- **验证（本轮，2026-09-14）**：新增 `TestPurgeStatementsStayIndexUsable`（对真实 purge SQL 断言命名到保留索引 + 无临时排序；修复前实测计划为 `SEARCH … USING COVERING INDEX idx_agent_control_agents_closed_at (closed_at>?) | USE TEMP B-TREE FOR ORDER BY`，修复后为 `SEARCH … USING COVERING INDEX idx_agent_control_agents_closed_at (closed_at>? AND closed_at<?) | CREATE BLOOM FILTER`，wake 语句同为 `SEARCH … USING COVERING INDEX idx_agent_control_agent_wake_created_at (created_at<?)`）与 `TestPurgeLeavesUndatableTimestampsAlone`（`closed_at='0000-00-00T00:00:00Z'` 这类「字典序很老但无法解析」的行**及其 wake 事件**即使落进新的可索引范围也必须留下，且同一 pass 仍要删掉可解析的老行/老事件——两侧的 `datetime()` 权威判定都跑到了）；新增 `BenchmarkPurgeTerminalAgentBatch10k`（10k 终态行、单批 512 行：新语句 `9.90 ms/op`；对照的旧语句 `37.13 ms/op`（约 3.7×）来自一次性对照夹具，未入库，此数仅为当时实测记录——入库可复跑的是当前语句基准。单 pass 至多 8 批，故一次对账的 GC 开销从 ~300ms 降到 ~80ms）。
...
- **待补（不阻塞主体）**：真机长跑 probe 未做（并发 spawn 不越限、空闲驱逐只作用于可安全回收对象）；retention GC 的万级规模已有可复跑证据（`BenchmarkPurgeTerminalAgentBatch10k`，见上方「retention 查询计划」）；配置迁移说明已补入 `MIGRATION.md` §9。产品事件流 `agent.reclaimed` 已接前端轨迹（白名单 + 一行 system note 摘要，见下方定向用例）。
...
- P2-9 retention 与宿主一致性定向用例（本轮新增）：`go test ./internal/agentcontrol -run "TerminalRetention|PurgeTerminal|ReconcilerFoldsPurge|ReconcilerRecordsPurgeFailure" -count=1`（8 例：窗口归一化与 `off`/`default` 解析语义、禁用或 store 不支持时 no-op、只删「已终态且早于窗口」的行与 wake 事件、分批收敛与 `MaxBatches` 上限、purge 结果并入同一份报告、purge 失败只记 `purge_error` 而不清空 drift 报告）；`go test ./cmd/aicli/commands -run "RegistryReconcile|RegistryTerminalRetention|LocalRegistry" -count=1` 与 `go test ./internal/api/skills -run "AgentRegistryTerminalRetention|AgentRegistryReconcile|AgentControl" -count=1`（两宿主同口径：env > runtime config > 默认、`off`/负值关闭 GC、非法值回退、nil 宿主安全；API 侧 `TestListAgentControlAgentsSweepsMissingSessionRows` 覆盖「会话已消失的 active 行在列表物化时被收敛为 stale，存活行不被误伤」）；`go build ./...` exit 0。
...
- 补充（2026-09-13，第七轮）：**真机 probe 发现并修复「spawn run 永不终态 → 假性 `progress_stalled`」（P0-4/P2-9/P2-10 的交集缺口）**。证据（本机 runtime 数据 `~/.aicli/sessions/runtime/supervision/supervision.db`）：`supervision_execution_runs` 中 `run_20260913115954_5aa14203`（p0-verify）、`run_20260913115954_0b9c243b`（p1-5-verify）、`run_20260913115955_42aed38c`（p1-67-verify）、`run_20260913115956_d25f05d6`（p2-verify）四条 `workflow=spawn_agent` 的 run，在对应子会话已经 `agent_completed`/`agent_failed` 之后仍停留在 `queued`（`finished_at` 为空、`progress_seq=0`、无 owner），于是 watchdog 每轮把 `progress_stalled` 投影给父会话；同时 `supervision_actions` 表为空，通知只增不减（N9 的真机形态）。注：`supervision_snapshot`/`ack_lifecycle`/`control_descendant` 三个模型侧工具其实已由 `cmd/aicli/commands/chat_actor_host.go:1394-1398`（`newLocalSupervisionToolController`）装配，只是不出现在静态工具列表、需经工具搜索发现；首轮排查据此误判为「宿主未暴露」。根因：`backend/internal/toolbroker/execution_run_hook.go:25`（`startSpawnExecutionRun`）注册 run 之后，没有任何宿主路径调用 `MarkExecutionRunTerminal`/`RecordExecutionProgress`（两 API 的调用点仅存在于 `internal/supervision` 内部与测试）。修复：`backend/internal/supervision/projection.go` 的 `ProjectAgentCompletion` 在投影子会话终态时，best-effort 收敛该子会话下仍 active 的 run（completed→`succeeded`、failed/error→`failed`、stopped/interrupted/canceled→`canceled`；已终态 run 不被改写；store 未实现 `ExecutionRunStore` 或 child id 为空时 no-op；run 写入失败不影响生命周期通知投影）。

  验证（backend 目录）：`go build ./...` exit 0；`go test ./internal/supervision -count=1` 绿；新增 `backend/internal/supervision/projection_run_finalize_test.go` 全绿（三状态映射、已终态不被覆盖、可选 store 与空 child id 退化）；宿主侧定向回归 `go test ./internal/api/skills -run "Spawn|Supervision|Rollback" -count=1`、`go test ./cmd/aicli/commands -run "Supervision|Completion" -count=1` 全绿。**历史通知收敛（同轮完成）**：7 条历史 unresolved 通知（2 条 `agent_failed`、4 条 `progress_stalled`、1 条 stale `subagent.batch.failed`）已用模型侧 `ack_lifecycle(decision=resolve, state=closed)` 逐条收敛，`supervision_snapshot(after_seq=0)` 复核 `critical_unresolved=0` / `action_required=0`（仅剩 `stale_subjects` 计数）；watchdog 随后仍把 4 条 run 通知的 version 推到 1041-1047，但 `resolution_state=closed` 未被回写成 unresolved —— 「决定」对重复投影是幂等的。**边界（未做）**：`control_descendant(action=cancel)` 对这批历史 run 被拒（`supervision: target agent is outside the requested root scope; not authorized`），故 `supervision_execution_runs` 中那 4 行仍是 `queued`；对已关闭子会话的 run 只能走 decision 收敛，`control_*` 只对「在册后代」生效——该边界值得补测试或写进 P2-12 文档。修复需重建 runtime-server/CLI 后才对新 spawn 生效（本轮宿主二进制 2026-09-13 19:57 已含模型侧工具、未含 ebcd24f9）。

- 补充（2026-09-14，真实 provider 验证）：**`scripts/validate-multi-agent-real-terminal.ps1` §4 判定口径修正 —— team probe 由「假红」改为证据判定**。背景：以 `opencode.ai / deepseek-v4.1-flash / reasoning_effort=max` 跑官方脚本（`output/real-test/config.team-probe.yaml` 局部配置，未改用户全局配置；因 `backend\aicli.exe` 被其它会话占用，用 `-AicliPath backend\aicli-probe.exe -SkipBuild`）——§2 smoke 通过、§3 spawn_agent 通过（2 个子 agent：`route=parent_inherit`、`difficulty=normal`、`fallback=0`、子会话 `message_count>=2`），但 §4 记为「未通过或证据不足」。实际证据：`session_history.sqlite` 父会话 `spawn_team@seq4 → wait_team@seq6`，`wait_team` 工具结果文本为 `Team <id> status=done. Team is terminal. Summary is ready. … Returned 21 lifecycle events.`；`team_store.sqlite` 的 `teams.status=done`、`team_events` 含 `team.completed`×1 / `team.summary`×1 / `task.completed`×3，三条 `agent_control_task_records` 均 `done` 且路由为 `opencode.ai/deepseek-v4.1-flash`。根因：§4 原先只扫描命令输出与 `session_*.json`（当前版本会话正文写入 `session_history.sqlite`，该 json 已不落盘），而 `team.summary` 是持久事件名，不会出现在 parent 最终正文。修正：新增 `Get-TeamTerminalEvidence`（查 `teams`/`team_events`）并复用 `Get-RecentRootSessions`/`Get-SessionToolCallNames` 取父会话 tool_calls，报告补 `Required evidence source` / `Parent session` / `Parent tool calls` / `Team row` 四行。验证：`Parser::ParseFile` 无错；仅重跑 §4（`-SkipProviderSmoke -SkipSpawnAgentProbe`）得 `Required found: wait_team, team.summary, spawn_team`、`Parent tool calls: spawn_team, wait_team`、`Team row: summary-team | status=done | team.summary=1 | team.completed=1`、forbidden 与环境阻断信号均 `<none>`，结论「真实 provider 非交互 spawn_team + wait_team 验证通过」。报告：`docs/working/multi-agent-real-terminal-validation-20260914-105959.md`（假红现场+追记）与 `…-110526.md`（修正后通过）。**未做**：人工交互项（Ctrl+C 两次语义、TUI `/agents` 面板 follow、`/collab`、`/timeline`）仍待真实终端勾选。
- 观察（同上，非缺陷，供后续确认）：①`team.summary` 首次投影曾落一条 `team.summary.failed`（`error: session is busy (running)`、`result_contract.status=partially_completed`），紧接着补发成功的 `team.summary`（`status=succeeded`），最终语义不受影响，属 retry/fallback 行为；②`agent_control_agents` 的 team 成员行（`agent_path=/root/teams/<team>/member-N`）由 `backend/cmd/aicli/commands/chat_actor_registry.go:1988` 的 `projectLocalTeamAgentRecords` 固定投影为 `AgentStatusActive`，且不写 provider/model/difficulty/route_source（路由证据在 `agent_control_task_records`）；`created_at` 因 `agent_id`（`team:<team>:<member>`）UPSERT 而沿用首见值（`ON CONFLICT(agent_id)` 的更新列不含 `created_at`，见 `backend/internal/agentcontrol/global_agent_store.go:282`/`:659`）。于是团队终态后成员行仍为 `active`，不进入按 `closed/stale` 过滤的 retention 清理口径 —— 是否应随 `team.completed` 收敛留作后续项。
...
- 未决项（实施前需确认）：
  1. ~~CLI `Spawn` 调用方在预留成功后的失败清理路径未逐行核实~~（2026-09-13 已核实：补偿覆盖 actor/prompt 失败，`snapshot` 读回失败分支当时未走补偿（口径未定）→ 已在 §10 E4 收口：按 `queued` 分流，两宿主同口径，见 §5 P0-3 实施注记）。
...
## 附录 C：单步「退化回复」重试放大复盘（2026-09-13）

- 现象：`session_20260913210527_7mj5o08M` 的单步耗时被拉到 3–6 分钟；模型每轮返回 `finish_reason=length` + `reasoning_content` 非空、`content` 为空、无 `tool_call`（即 `reasoning_only_empty_reply`，模型把整个 completion budget 花在推理上）。
- 既有机制（复盘时不重复实现）：`escalateOutputBudgetForReasoningOnlyReply()` 把 max_tokens 翻倍（上限 32768、最多 2 次），`degenerateOutputRetryMaxDelay = 2s` 把该类的退避封顶——两者都只作用于「同一调用内」的重试。缺口在于 `isDegenerateOutputRetryReason()` 覆盖的 `reasoning_only_empty_reply` 同时是 `isHandoffEligibleError` 的 handoff 目标，于是三层循环（provider 非流式 / provider 流式 aggregate / gateway）会把同一条退化请求各自重放满 `MaxAttempts`（实测 attempt 到 10、每步 ~64s），一次「模型没写出东西」被放大成分钟级卡顿。
- 修复（阈值 `degenerateOutputReplyMaxStreak = 3`）：`backend/internal/llm/provider_retry.go` 新增 `trackDegenerateOutputReply(*int, error)` —— 非退化 reason（含 `nil`）重置 streak，退化 reason 累加，达到 3 次返回 true；`provider.go` 两处循环与 `gateway_client.go` 一处循环在「预算翻倍已用完」之后调用它，命中即 `markRetryExhausted(...)` 立刻终止，消息分别为 `provider call aborted after repeated degenerate replies`、`streaming aggregate call aborted after repeated degenerate replies`、`gateway call aborted after repeated degenerate replies`；流式路径若已吐出部分内容仍打 partial output 标记。
...
### 附录 D 补充：作业与 shell 结果的目录一致性（2026-09-15，第三轮）

附录 D 统一了「策略 ↔ 执行器 ↔ 预检」在**单次工具调用**内的相对路径基准。本轮继续沿同一条不变量下探，发现残留缺口都出在**同一解析链的下游消费者**：路径被正确解析之后，仍有人在用解析前的原始参数去「描述」或「落地」这次执行。

#### D-3.1 `background_task` 的 `cwd` 未按会话工作区根锚定（已修）

- 根因：`internal/toolbroker/broker.go:1235-1280` 把 `args["cwd"]` 原样塞进 `BackgroundTaskArgs.Cwd`，`internal/background/manager.go:237` 落到 `Job.Cwd`，真正生效点是 `manager.go:673-675` 的 `cmd.Dir = managed.info.Cwd` —— 即**相对/缺省 cwd 相对服务器进程目录解析**，与 `exec.Cmd` 的默认行为一致。而策略层（`internal/policy/tool_policy.go` 的 `policyPathArgKeys` 已含 `cwd`/`workdir`/`working_dir`，解析走 `resolvePolicyPath` → ctx 会话根）是按**绑定目录**放行/拒绝的。两处不一致时：策略校验的是 A 目录，作业跑在 B 目录，属 fail-open。
- 修法（最小面）：新增 `internal/background/cwd_resolution.go` 的 `resolveJobCwd(ctx, cwd)` —— 绝对 `cwd` 原样优先（仅 `filepath.Clean`）；否则以 `toolctx.WorkspaceRoot(ctx)` 为锚点（相对锚点先 `filepath.Abs` 再 `Clean`）；`cwd` 为空时**返回锚点**，让作业落在会话工作区而不是服务器进程目录；无锚点时原样返回，保持无 ctx 场景的旧行为。接线一处：`manager.go` 在 `req = sanitizeBackgroundTaskArgs(req)` 之后执行 `req.Cwd = resolveJobCwd(ctx, req.Cwd)`。
- 回归测试：`internal/background/cwd_resolution_test.go` —— `TestResolveJobCwdAnchorsRelativeAndEmptyToSessionRoot`、`TestResolveJobCwdKeepsLegacyBehaviorWithoutSessionRoot`、`TestResolveJobCwdNormalizesRelativeSessionRoot`、`TestSubmitShellAnchorsJobCwdToSessionRoot`（后者同时断言 `requestFromJob(*job).Cwd` 保持锚定值，避免重启/重试回放时退化回进程目录）。
- 边界：`internal/policy/tool_policy.go:105-108` 已在只读策略下整体拒绝 `background_task`，所以本改动只是在已放行的调用内收窄落点，不放宽任何权限。

#### D-3.2 shell 工具回显/提示的 `Workdir` 用的是解析前参数（已修）

- 根因：`internal/toolkit/tools/bash.go` 的 `Execute` 用**原始参数** `workdir := extractString(params["workdir"])` 去调用 `formatShellCommandContent(...)`（3 处：正常退出、非零退出、rg/grep exit 1 空证据分支）与 `friendlyHintFor(command, output, err, workdir)`（2 处），而命令实际跑在 `b.workdirForExecution(ctx, workdir)` 解析出的目录里（`executeCommand` 内部再解析一次，幂等）。后果与上一轮"路径候选写错树"的症状同源：省略 `workdir` 时模型看到的头部**完全没有 `Workdir:` 行**（`formatShellCommandContent` 对空值跳过该行），且相对路径提示（如"可能的路径候选"）以服务器进程目录为基准 —— 会话实际绑定在别处时，模型会顺着提示去找**另一棵树**。
- 修法：`Execute` 在取到 `workdir` 后立即解析一次
  `effectiveWorkdir, workdirErr := b.workdirForExecution(ctx, workdir)`（失败即时返回 `Success:false` + 该错误），随后 `executeCommand`、两处 `friendlyHintFor`、三处 `formatShellCommandContent` 一律改用 `effectiveWorkdir`。语义上"策略校验的目录 = 命令执行的目录 = 结果头部与提示声称的目录"三者合一；显式绝对 `workdir` 仍原样优先。
- 回归测试：`internal/toolkit/tools/bash_workdir_test.go` 新增 `TestBashExecuteReportsResolvedSessionWorkdir`（用 `inspectExecuter` 记录 `WithWorkdir` 实参，两个子用例：省略 `workdir` → 头部报会话根；相对 `workdir` → 头部报 join 后的绝对目录，同时断言执行器收到的 workdir 与头部一致）。
- 同族核查（无需改动）：`internal/toolkit/tools/aicli_exec.go` 已是解析后使用 —— `:217` 走 `t.workdirForExecution(ctx, req.CWD)` 并 `cmd.Dir = cwd`（`:228`），元数据也传解析后的 `cwd`（`buildAICLIExecMetadata(commandForMetadata, cwd, ...)`，`:255`）；其 `Content` 为子进程原始输出，不含工作目录头部，故不存在第二套描述。

#### D-3.3 本轮验证（backend 目录）

- `go build ./...` exit 0。
- `go test ./internal/toolkit/tools/ -count=1` → ok（12.86s）；`go test ./internal/toolbroker/ ./internal/background/ -count=1` → ok（9.32s / 20.64s）。
- `go vet ./internal/toolkit/tools/ ./internal/background/ ./internal/policy/` 无输出。
#### D-3.4 `Spawn` 快照失败未补偿 durable 预留（N1 残留分支，已修）

- 根因：`internal/api/skills/session_runtime_support.go` 的 `Spawn` 在 `reserveOrRegisterAgentSpawn` 成功之后就带有明确不变量（注释写在 `:593-596`：任何让子会话不可用的失败都必须补偿预约），并且已经为 `GetOrCreate`（`:614-617`）与 `SubmitPromptAsync`（`:619-623`）失败实现了 `rollbackSpawnFailure`。但紧随其后的 `snapshot` 失败分支（原为 `return nil, err`）遗漏了同一补偿。
- 危害面：只落在「spawn 但不带 message」这一支 —— `queued == false` 意味着没有任何 run 会把这笔预约推向终态，残留的 active 行会永久占用 `agents.maxThreads`（正是 N1 描述的配额僵尸）。反向一支（已有 prompt 在跑）不能删子会话，否则等于杀掉一个活着的 agent，保持只报错是正确语义。
- 修法：`snapshot` 失败时按 `queued` 分流 —— `!queued` 走 `rollbackSpawnFailure(fmt.Errorf("snapshot child session: %w", err))`（复用同一补偿路径：清理隔离 → 停 actor → 删会话 → 释放预约），`queued` 仍只返回诊断原因，并补充注释说明为何不回收。
- 回归测试：`internal/api/skills/session_agent_spawn_snapshot_rollback_test.go` 的 `TestSessionAgentController_SpawnRollsBackReservationWhenSnapshotFails`。故障注入手法可复用：team store 走懒加载 SQLite，用「目录当库文件」的路径让首次真实操作必然失败，并先探测一次确认注入生效；该失败点只出现在 `snapshot` 的 team 投影（`enrichAgentTeamProjection` → `team.FindTeammateBySession` → `Store.ListTeams`），早于它的 actor 创建、晚于它的 prompt 排队都不受影响。用例断言错误被包装为 `snapshot child session`、子会话已删除、registry 行落到 `stale`+closed、active 集合只剩 root 行。
- 反证（先证明用例有效再落地）：把新分支临时置为不生效后运行同一用例，失败信息为原始 `open team db: ... unable to open database file`（不含 `snapshot child session`），即修复前该用例确实红；恢复分支后转绿。
- 追加验证：`go test ./internal/api/skills/ -count=1` → ok（14.46s）；`gofmt -l` 对 `session_runtime_support.go`、`session_agent_spawn_snapshot_rollback_test.go`、`bash.go`、`bash_workdir_test.go`、`background/cwd_resolution*.go` 均无输出。

---

## 附录 E：协作引导落进工具描述 + 监督动作面补齐 CLI 入口（2026-09-15）

### E1 CLI `/debug supervision control`：交互操作者的收敛入口（P2-12 ①）

- 缺口：模型侧 `control_descendant` 与 HTTP `/api/runtime/supervision/actions` 都能跑 cancel/close/cancel_subtree/retry/reassign，但交互式 CLI 操作者只能 `ack|defer|resolve`。一条 critical 通知若标的是卡住的子会话，人在终端里没有本地收敛路径（只能等模型自己发起调用），而 ack 会把「已处理」写成假事实。
- 实施：`backend/cmd/aicli/commands/chat_debug_supervision.go` 新增 `control` 子命令（`runChatDebugSupervisionControl` + `parseChatDebugSupervisionControlAction` / `parseChatDebugSupervisionCascade` / `chatDebugSupervisionControlService`），复用 `supervision.NewLocalControlService`，与模型侧同一实现：
  `/debug supervision control <notification_id> --action cancel|close|cancel_subtree|retry|reassign|inspect --reason <text> [--cascade target|descendants] [--expected-version N]`
- 口径：
  - `--reason` 必填（动作会持久化为 action 行，无理由不可审计）；`inspect` 之外的动作按 request→accept→execute 落 durable 行，输出 `action_id`/`status`/`cascade` 供 `list` 复核。
  - 本地宿主未接线 action service 时给出明确错误（`本地宿主未接线 action service：control 无法执行`），不静默降级为 ack。
  - CAS：沿用 `--expected-version`；每个动作会为同一标的写入解析通知并推进版本，连续操作前需重新 `list`（已写入 `docs/user-guide/aicli.md` §4.8）。
- 验证（backend）：`go test ./cmd/aicli/commands -run TestChatDebugSupervisionControl -count=1` 定向通过；`go test ./cmd/aicli/commands -count=1` → ok（76.47s）；`go vet ./cmd/aicli/commands` exit 0；`gofmt -l` 无输出。用例覆盖：动作解析（未知动作/缺 reason/缺 id 的拒绝）、宿主未接线时的错误、scope 外的通知被拒、成功动作落 durable 行、沿用旧通知行的 `--expected-version` 冲突被拒。
- 边界：`control` 只对「在册后代」生效；对已关闭子会话/历史 run 的标的仍会被拒（`target agent is outside the requested root scope`），这类行只能走 decision 收敛（`ack|defer|resolve`）。

### E2 协作纪律与等待预算的「一源两处」（P2-11 方案 2+3）

- 缺口：等待窗口的边界是**运行时事实**（`agentcontrol.ResolveWaitTimeout`：默认 30000ms、下界 10000ms、上界 3600000ms、`agents.waitTimeoutMode=error|clamp`，两个宿主都在用），但工具描述层只写 "Optional wait timeout in milliseconds."；同时「等到最长可承受窗口 / 超时后先做独立工作」这条纪律在 `internal/prompt` 的系统引导与 `internal/toolbroker` 的工具描述里各存一份，已经漂移过（提示词说了、schema 没说）。
- 实施：
  - 新增叶子包 `backend/internal/agentguidance`（无内部依赖）：`WaitBudgetRule`、`WaitEscalationRule`（新增纪律：只改 timeout 不算新工作）、`WaitResultEchoNote`、`EventsWaitNonBlockingNote`，以及 `WaitTimeoutArgText(...)`/`EventsWaitArgText(...)`/`WaitDisciplineText()`。
  - `internal/prompt/environment_context.go` 的协作引导改为引用同一常量，并新增「抬升 timeout 不是进度」一条。
  - `internal/toolbroker/broker.go` 四处 schema/描述接线：两个 `wait_agent` 变体的 `timeout_ms` 与两个 `read_agent_events` 变体的 `wait_ms` 都改由共享函数渲染（含默认值、上下界、error/clamp 语义、`wait_timeout_clamped` 回显；`wait_ms=0` 保留「非阻塞读」语义描述），`wait_agent` 描述追加共享纪律。
- 反证（先证明用例有效再落地）：把第一个 `wait_agent` 变体的 `timeout_ms` 临时改回旧文案，`TestCollaborationToolSchemasShareWaitBoundsAndDiscipline` 与 `TestCollaborationToolSchemaSourceHasNoLegacyWaitText` 同时变红（分别报 `wait_agent timeout_ms must describe the enforced bounds` 与 `legacy wait text still present`）；恢复后转绿。
- 验证（backend）：`go build ./...` exit 0；`go test ./internal/toolbroker ./internal/prompt ./internal/agentguidance -count=1` → ok（9.15s / 1.42s / 0.49s）；`gofmt -l` 无输出。守卫设计：运行期守卫遍历两种宿主配置下可达的定义做逐字段比对，源码守卫逐条检查 `broker.go` 里**每个** `wait_agent`/`read_agent_events` 定义都必须引用共享函数（覆盖测试无法构造的宿主变体），二者与共享常量共同保证「只改一个变体」不可能通过。
- 补充闭环（2026-09-15）：`wait_team` 并入同一等待预算，上文「边界（未做）」的第一项已消除。
  - 缺口：`wait_team` 的观察窗口在 `executeWaitTeam` 里自带 30000ms 默认值，既无上下界也无 `waitTimeoutMode`——「换一个等待工具」即可绕过 `agents.maxWaitTimeoutMs`，与 P2-11「任何路径的等待时长都落在 [minWaitTimeoutMs, maxWaitTimeoutMs]」直接矛盾。
  - 实施：`Broker.WaitTimeoutPolicy func() agentcontrol.WaitTimeoutPolicy`（宿主注入、每次调用惰性求值）；`executeWaitTeam` 在**任何副作用之前**调用同一个 `agentcontrol.ResolveWaitTimeout`，越界请求按 mode 拒绝（带边界值与逃生口的可执行文案）或钉到最近边界；`WaitTeamResult.WaitTimeoutRequestedMs/WaitTimeoutClamped` 与 raw metadata 三件套回显，与 `wait_agent` 同口径（请求值、生效值、是否被钉）；`wait_team` schema 的 `timeout_ms` 与描述改由 `agentguidance.WaitTimeoutArgText(...)`/`WaitDisciplineText()` 渲染，与提示词同源。
  - 宿主接线：API `Handler.applyAgentRuntimeServices` 用 `defer` 盖章（覆盖含 `config==nil` 早退的每个分支）→ `h.brokerWaitTimeoutPolicy()`；CLI `buildLocalChatAgent` → `host.ActorRegistry.localWaitTimeoutPolicy`。两侧都是各自 `wait_agent`/`read_agent_events` 用的那一个 mapper，不新增第二份字段清单。
  - 反证（先证明用例有效再落地）：临时移除 API 宿主 `broker.WaitTimeoutPolicy = h.brokerWaitTimeoutPolicy`，新增守卫 `TestApplyAgentRuntimeServicesStampsBrokerWaitTimeoutPolicy` 立即变红（`the broker must resolve wait_team windows from the live agents policy`）；恢复后转绿。
  - 验证（backend，终态复跑）：`go build ./...` exit 0；`go test ./internal/toolbroker ./internal/api/skills ./cmd/aicli/commands ./internal/agent ./internal/chat ./internal/agentcontrol ./internal/prompt -count=1` 全绿（18.4s / 24.6s / 92.2s / 11.4s / 20.5s / 5.1s / 3.1s）；本轮改动/新增的 Go 文件 `gofmt -l` 无输出（`broker.go`/`types.go` 因新增字段产生的字段对齐已在本轮 `gofmt -w` 收敛；同目录其余被 `gofmt -l` 列出的文件为 HEAD 既有债务，未扩大）。用例（`broker_team_wait_policy_test.go`）：上界钉住、下界抬升、缺省窗口（`requested=0` 且不误报 clamp）、界内原样且不误报 clamp、error 模式两侧拒绝且文案含边界与逃生口、provider 每次调用重读（clamp→error 翻转即时生效）；守卫（`broker_wait_schema_guard_test.go`）：运行期逐字段比对与源码逐条检查都延伸到 `wait_team`（含旧文案黑名单），新增的 `team-host` 配置让「没有宿主暴露该工具」这种空转守卫直接失败；两宿主各加一条接线守卫（并断言 provider 读的是**当下**配置而非装配期快照）。夹具用已终态 team（`team.completed`+`team.summary`）保证断言只关于窗口归一化，不引入墙钟等待。
  - 边界（仍未做）：真实终端 probe 未做（`scripts/validate-multi-agent-real-terminal.ps1` 尚未覆盖 `wait_team` 越界请求）。

### E3 宣告层按宿主能力收窄（P2-12 方案 1）

- 缺口（本轮根因）：动作集合是**宿主中立**的（`Evaluator.EvaluateAllowedActions` 描述「标的允许什么」，durable 行与 enforcement 必须如此），但 digest / snapshot / preflight / wake 是**宣告**——宣告的必须是「这个宿主真的能执行的动作」。历史症状：无 wired executor 的宿主照样宣告 `cancel/close`，模型据此规划 `control_descendant`，得到 `no executor configured`，父回合白跑一轮。更深一层的接线缺陷：`runtimeserver.BuildSupervisionControlPlane` 在 `hooks.Execute == nil` 时仍装了一个**非 nil** 的 `runtimeActionExecutor{execute: nil}`，使 `ActionService` 在类型层面无法区分「已接线」与「未接线」，`ExecutorReady` 无从如实禀报。
- 口径分层（执行层不动）：
  - 执行层（enforcement 与 durable 行）= `EvaluateAllowedActions(n)`，宿主中立；
  - 宣告层（digest / snapshot / preflight / wake / 模型工具）= `EvaluateAllowedActionsForHost(n, caps)`，在宿主中立集合上**只做减法**，并在 `NextAction` 说明被减掉的原因；`nil` 表示宿主未申报能力 → 返回原集合（接线缺失永不隐藏真实修复路径）。
- 实施：
  - 新增 `backend/internal/supervision/capabilities.go`：`HostCapabilities{DecisionActions,ControlActions}`、`FullHostCapabilities()`、`Supports()`、`EvaluateAllowedActionsForHost()`、`ActionCapabilityHint()`，以及两个稳定 hint 常量 `ack_requires_local_command` / `control_requires_wired_action_executor`（`inspect` 恒可达：它是读，由 digest/snapshot 本身提供）。
  - 宣告面接线：`DigestRequest/SnapshotRequest/LocalSnapshotRequest.HostCapabilities`；`DigestItem/SnapshotItem.NextAction`（JSON `next_action`）与 digest 文本 `; next_action=...`；stale 行同时清空 `NextAction`。
  - `ActionService.ExecutorReady()`（`SetExecutor(nil)` 会翻回 false）+ `BuildSupervisionControlPlane` 在 `hooks.Execute == nil` 时传 nil（服务装 noopExecutor，如实申报 `ControlActions=false`）。
  - 宿主申报：HTTP `supervisionHostCapabilities()` → preflight digest / `GET digest` / `GET snapshot` / 临时 wake scheduler；CLI `localSupervisionHostCapabilities()` → 本地 preflight 与模型侧 `supervision_snapshot`；wake 路径经 `WakeSchedulerConfig.HostCapabilities func() *HostCapabilities` 惰性求值（CLI 与 runtime-server 都是「先建控制面、后 `SetActionExecutor`」，构造期快照会报错的口径），覆盖 `DrainRunnable` 与 turn-end `RootDigest`。
- 反证（先证明用例有效再落地）：临时注释掉 `DrainRunnable` 里的 `HostCapabilities: s.hostCapabilitySnapshot()`，`TestWakeDigestFiltersAnnouncedActionsByHostCapabilities` 变红（actual `[inspect,acknowledge,defer,cancel,close]`，expected `[inspect,acknowledge,defer]`）；恢复后转绿。
- 验证（backend）：`go build ./...` exit 0；`go test ./internal/supervision -count=1` ok；`go test ./cmd/aicli/commands -run "Supervision|Wake" -count=1` ok；`go test ./internal/api/skills -run "Supervision|Wake" -count=1` ok；本轮改动/新增文件 `gofmt -l` 无输出（`snapshot.go` 的字段对齐为 HEAD 既有 gofmt 债，未扩大）。用例覆盖：能力矩阵五种组合的集合与 hint、deferred/acknowledged/resolved 行「无可减则不产生 hint」、digest 文本与结构体同源、snapshot standalone 分支、`ExecutorReady` 翻转、wake digest 的惰性刷新，以及「宣告永不放大权限」（所有被宣告动作都必须属于宿主中立集合）。
- 边界（未做）：`stale` 判定之外的 `SubjectPresence` 语义不变；`/debug supervision list` 的单条 `allowed=[...]` 仍是宿主中立集合（它陈述 durable 行事实，且本地 CLI 两条通道都有入口：decision 内建、control 走 action service）；真实终端 probe 未做。

### E4 P0-3 收口：`snapshot` 分支两宿主同口径 + 三条补偿路径的故障注入

- 缺口：P0-3 的补偿此前只覆盖 `GetOrCreate` 与 `SubmitPromptAsync` 失败；`snapshot` 读回失败分支两侧行为不一致——API 已按「有没有 prompt 在跑」分流，CLI 仍无条件 `return nil, err`（本地宿主照旧会留下永久 active 的僵尸预约，因为没有 run 会把它推向终态）；同时「`SubmitPromptAsync` 失败注入」「CLI 同场景」「名额已满」三处验收缺口未补。
- 口径（本次定案并写入 §5 P0-3）：**按 `queued` 分流**。没有排队任何 prompt ⇒ 不存在会把这笔预约推向终态的 run ⇒ 必须走 `rollbackSpawnFailure`（清理隔离 → 停 actor → 删会话 → 释放预约）；已有 prompt 在跑 ⇒ 只上报诊断原因，不删除子会话（删一个活着的 agent 比留一条待收敛的行更糟），由完成回调收敛预约。两宿主同措辞（`snapshot child session: %w`）。
- 实施：
  - CLI `backend/cmd/aicli/commands/chat_actor_registry.go` 的 `agentSnapshot` 失败分支与 API 对齐（同一分流、同一包装文本、复用同一 `rollbackSpawnFailure`）。
  - 新增 CLI 验收 `backend/cmd/aicli/commands/chat_actor_spawn_rollback_test.go`：快照失败回滚（断言包装文本、半成品会话删除、行落 `stale`+closed、active 只剩 root）、已排队不回滚（断言未被包装 + 子会话仍在）、名额已满不留行（断言被拒的 spawn 不落任何 registry 行且已占额度的子会话不受影响）。
  - 新增 API 验收 `TestSessionAgentController_SpawnRollsBackReservationWhenPromptQueueFails`（`session_agent_spawn_rollback_test.go`），与既有 `GetOrCreate` 失败、`snapshot` 读回失败两例凑齐三条补偿路径。
- 注入手法（两个新工具，均可复用）：
  - API 侧「创建即 Stop 的 actor」：hub 工厂返回 `NewSessionActor` 后立即 `Stop()` 的 actor。`SessionHub.GetOrCreate` 存下工厂返回值时不再复查 `IsStopped`（`hub.go:159-169`，而 `Get` 只清理**已在 map 中**的停止 actor），因此 `SubmitPromptAsync` 的 `send` 稳定返回 `ErrSessionActorStopped`，失败点精确落在排队一步。
  - CLI 侧 `spawnSnapshotFailureStore`（会话存储装饰器）：直接换「不可用的 team store」会把错误打到预约之前的物化投影上（投影同样读 team store，`Spawn` 在预约前就会 `materializeLocalAgentRegistry`），测不到补偿分支；因此改成「子会话 actor 已建出 + 开关打开」两个运行期事实同时成立时 `Load` 才失败——此时唯一会 `Load(childID)` 的调用就是 `agentSnapshot`，且与 API 用例覆盖同一分支。用例断言前 `disarm()`，注入器不干扰断言自己的读取。
- 反证（先证明用例有效再落地）：
  - CLI：把 `!queued` 分支的回滚改回 `return nil, err` → `TestLocalActorRegistrySpawnRollsBackReservationWhenSnapshotFails` 变红（`expected wrapped snapshot failure, got injected snapshot read failure`）；恢复后转绿。
  - API：把 queue 分支改回 `return nil, err` → `TestSessionAgentController_SpawnRollsBackReservationWhenPromptQueueFails` 变红（`"session actor is stopped" does not contain "queue child prompt"`）；恢复后转绿。
- 验证（backend）：`gofmt -l` 对改动/新增文件无输出；`go build ./...` exit 0；`go test ./cmd/aicli/commands -run "LocalActorRegistrySpawn" -count=1` 全绿（含 3 个新用例与既有 spawn/worktree/route 用例）；`go test ./internal/api/skills -run "SpawnRollsBackReservation" -count=1` 三例全绿；`go test ./cmd/aicli/commands ./internal/api/skills -count=1` 两包全量回归见本节末尾记录。
- 边界（未做）：两宿主的回滚**写序**不同（CLI：清理隔离 → 释放预约 → 停 actor → 删会话；API：清理隔离 → 停 actor → 删会话 → 释放预约，API 注释记录释放必须是最后一次写入，否则并发物化会按会话存储把行写回 active）。两宿主的物化投影都带同形守卫「已终态行不被 active 投影覆盖」（`materializeLocalAgentRegistry` / `materializeAgentControlAgentProjections`），CLI 顺序在静态与用例层面均未见可复现风险，本轮不改动写序、仅记录为待观察项；真实终端 probe 未做。
