...
| 项 | 状态 | 代码落点 | 定向测试 |
| --- | --- | --- | --- |
| P0-1 wait 超时边界 | 已实施 | 公共解析 `backend/internal/agentcontrol/wait_timeout.go`；API 接入 `backend/internal/api/skills/session_runtime_support.go:1799,1863,2014`；CLI 接入 `backend/cmd/aicli/commands/chat_actor_registry.go:2234,2298,2442`；结果回显 `backend/internal/toolbroker/types.go:489-493,590` | `go test ./internal/agentcontrol -run WaitTimeout -count=1` |
...
- P2-10 测试与验证补齐：只落地了随 P1-7 产生的读窗口分页用例（`backend/internal/toolbroker/types_pagination_test.go`），runbook 真机 probe 未做。
- P2-11 协作引导：**已实施**（`internal/prompt` 新增 `RenderMultiAgentCollaborationGuidance`，API 与 CLI 两宿主注入 system instruction；工具描述层本就带 spawn/wait 契约，双层齐备）。**未做**的是 ① 真机验证引导对模型行为的实际影响（spawn 后是否先做独立工作、是否减少同窗口重读）② `internal/agent` 子代理 prompt 的协作段落（默认 `maxDepth=1` 时子代理无 spawn 能力，暂不注入）。

**P1-7 行为口径**

- **软护栏**：`wait_agent` / `read_agent_events` / `list_agents` 等轮询工具仍豁免语义重复检测（不误判 doom loop）；新增 `PollingBackoffTracker` 按「同一批轮询调用 + 归一化参数摘要（含 target ids / after_seq / timeout_ms）」计连续次数，达到阈值（默认 3，`agents.maxRepeatedPollCalls` 可调，负数关闭）后向模型注入 `polling_backoff` system-reminder，**只提示不拦截**；批次中出现任何真实工作（非豁免工具）或参数变化即重置计数，产品事件 `tool_loop.polling_backoff_observed` 每段连续只发一次。
...
- **读窗口元数据（已实施）**：`read_agent_events` 返回 `latest_seq` / `has_more` / `unread_count`（`backend/internal/toolbroker/types.go` 的 `ApplyAgentEventsPagination` / `CapAgentEventsUnread`），CLI 本地 host 的剩余计数由 `localAgentEventsUnreadCount`（`backend/cmd/aicli/commands/chat_actor_registry.go:3687`）提供；工具描述明确 `has_more=true` 时先消费返回事件，再以 `after_seq=latest_seq` 续读。未读计数不可得时仍返回 `has_more` 并提示推进 `after_seq`，不阻塞调用方。
...
- **分层预算**：wake reason 归一为三类（`backend/internal/supervision/wake_budget.go:47`）——`approval`（`approval_required` / `question_asked` / `permission_requested`）、`failure`（`execution_failed` / `execution_timeout` / `lifecycle_failed` 及含 failed/stalled 的 reason）、`other`（其余，含默认 `critical_lifecycle`）；三类各自独立计费，密集失败不会挤占审批。
...
- **账本模式**：`wake_budget_mode: memory|durable`（默认 `memory`，保持历史行为）。`durable` 时每次已投递的 auto-wake 追加一行 `supervision_wake_claims`（`sqlite_store.go:251-261,1212`），窗口用量按 `root_scope_id + budget_class + claimed_at` 统计（`:1244`），因此多进程共享同一 DB 时预算一致、重启不清零；写失败降级为进程内窗口（best-effort），不会让已投递的回合失败。
...
- 未决项（实施前需确认）：
  1. CLI `Spawn` 调用方在预留成功后的失败清理路径未逐行核实（本轮只核实了 reserve 与 stale 清理存在）；P0-3 需先补这一步核查。
  2. 前端是否存在非 grep 命中形式的子会话入口（例如动态路由/面板），需一次 UI 走查确认。
  ...
  5. N9/P2-12 涉及的动作集合口径来自两处（durable `allowed_actions_json` 与注入摘要宣告），摘要侧的计算入口尚未逐行核实；实现前需先定位该计算点。
