...
- P2/P3 AgentControl mailbox 持久表与独立 sequence: `SQLiteRuntimeStore` migration v10 新增 `agent_control_mailbox_messages`，`AppendMailbox` 在写入 `session_mailbox_messages` 的同一事务内，会把携带 `message_type/control_action/workflow/mailbox_delivery/mailbox_kind` envelope 的 control message 同步写入该表；migration v11 进一步为该表增加 per-session `seq`，并为历史 v10 数据按 `session_mailbox_seq` 回填 control sequence。control row 现在同时保留 `seq`（AgentControl control cursor）与 `session_mailbox_seq`（兼容 session mailbox cursor），字段包含 `session_id/seq/session_mailbox_seq/message_id/team_id/from_agent/to_agent/task_id/kind/message_type/control_action/workflow/mailbox_delivery/mailbox_kind/body/metadata_json/created_at`。这让 `GET /agent-control/mailbox` 和 `AgentControlMailboxSequenceStore` 不再依赖普通 session mailbox seq，同时仍保持现有 session mailbox API 完全兼容。
- P2/P3 AgentControl mailbox 可读/可写 substrate: `chat` 新增 `AgentControlMailboxReaderStore` / `AgentControlMailboxWatcherStore` / `AgentControlMailboxSequenceStore` / `AgentControlMailboxStore` 与 `IsAgentControlMailboxMessage`。`SQLiteRuntimeStore` 已能从 `agent_control_mailbox_messages` 按独立 control `seq` 增量读取 control message，并提供 control high-water mark；`InMemoryRuntimeStore` 也维护独立 `controlMailbox/controlSeq`，不再只是每次过滤普通 mailbox。两种 store 都提供 AgentControl mailbox watch，通知会在底层 mailbox wake 后通过 AgentControl reader 做 durable catch-up，因此 watcher 返回的是带 `control_seq/session_mailbox_seq` 的 control row，而不是普通 session mailbox mirror。新增 `AppendAgentControlMailbox` canonical writer surface，要求消息携带 AgentControl envelope；`DeliverMailboxStoreFirst` 和公共入口 `DeliverMailboxEventFirst` 在发现 mailbox message 带 envelope 且 store 支持该接口时，会优先走 AgentControl writer，再由 runtime store 负责兼容 session mailbox/event mirror。新增 `chat.ListMailboxAgentControlFirst` / `chat.WatchMailboxAgentControlFirst`，把“AgentControl control rows 优先、legacy session mailbox rows 兼容合并、durable high-water mark 判断、watch 去重”的逻辑从 CLI/API 两套实现收口为共享 helper；parent mailbox 合并视图使用 `session_mailbox_seq` 去重和排序，AgentControl-only API 使用 `seq/control_seq` 作为 cursor。Runtime HTTP 进一步新增 `GET /api/runtime/sessions/{id}/agent-control/mailbox`，直接返回 AgentControl mailbox row，而不是转换成 `mailbox_received` runtime event；typed client 新增 `ListSessionAgentControlMailbox`，`skillsapi-demo -mode session-agent -agent-action control-mailbox` 可直接读取 control mailbox substrate。`/collab` 现在会优先探测 AgentControl mailbox reader，并在 header 中标出 `source=agent_control+mailbox` 与 `control_events`，同时仍补全普通 mailbox 行和非 mailbox 协作终态事件，便于验证新 substrate 是否真的有数据。`wait_agent/read_agent_events` 的 parent mailbox 模式现在也会优先读取/订阅 AgentControl mailbox substrate，再与旧 session mailbox 行按 `session_mailbox_seq` 合并去重，因此 control message 已经进入主等待/读取路径；旧普通 mailbox 行仍保留兼容返回。
...
- P3 team mailbox durable wake AgentControl seam: `agentcontrol` 新增 `MailboxWakeFilter` / `MailboxWakeEvent` / `MailboxWakeWatcher` / `MailboxWakeSequencer` / `MailboxWakeSource` seam，`team.AgentControlMailboxWake` 会把 team mailbox `WatchMail/LastMailSeq` 投影为 AgentControl mailbox wake stream。`Orchestrator.RunWithWake` 不再直接依赖 team `MailWatcherStore/MailSequenceStore`，而是消费 `agentcontrol.MailboxWakeSource`，并用 AgentControl mailbox wake sequence 做 durable high-water mark；默认 source 仍由 team adapter 提供，也可注入后续全局 AgentControl mailbox substrate。
...
- `go test ./internal/api/skills ./pkg/skillsapi ./internal/team ./internal/agentcontrol -run "Test(AgentControlTaskWriteHandlersUseTaskRegistrySeams|AgentControlTaskRegistryCreatesTaskDependency|AgentControlTaskRegistryListsTaskDependencies|SQLiteStoreListTaskDependencyRecords|TaskDependencyCreateRequestNormalize|TaskDependencyRecordAndFilterNormalize|Client_AgentControlTaskWriteEndpoints|Client_ListAgentControlTaskDependencies)" -count=1 -v`
...
- team mailbox 已具备 durable sequence/watch 基础，并且 team mailbox 到 session mailbox 的 dispatch 已是 mailbox-first；`wait_agent` 无目标调用与 runtime HTTP `/agents/wait` 空目标请求均已支持等待当前 parent session mailbox/collab event，`read_agent_events` 无目标调用与 runtime HTTP `/agents/events` 也已支持读取 parent mailbox/collab event，二者优先使用 mailbox substrate 并返回 session mailbox `latest_seq` cursor，session event 只作为兼容 fallback。底层已有 `MailboxStore`/reader/watcher/sequence substrate 接口，runtime stores 也已原生实现 `AppendMailbox/ListMailbox/WatchMailbox/LastMailboxSeq`，SQLite 默认持久层已有独立 `session_mailbox_messages` 表，并新增 `agent_control_mailbox_messages` 持久 control row；AgentControl mailbox 也已有 writer/reader/watcher/sequence 接口和 runtime store 实现，且 control row 有独立 `control_seq`，control-only API 不再复用 session mailbox seq 作为 cursor。`DeliverMailboxStoreFirst` 对带 envelope 的 control message 会优先走 `AppendAgentControlMailbox`，`chat.ListMailboxAgentControlFirst` / `chat.WatchMailboxAgentControlFirst` 已把 CLI/API 的 parent mailbox 双读兼容逻辑集中到共享 helper，`/collab`、`wait_agent` parent mailbox 模式和 `read_agent_events` parent mailbox 模式已开始消费该 substrate，并与旧 session mailbox 按 `session_mailbox_seq` 合并兼容。Runtime API 现在也有显式 `GET /api/runtime/sessions/{id}/agent-control/mailbox`、typed client `ListSessionAgentControlMailbox` 和 `skillsapi-demo` 的 `control-mailbox` action 可直接读取 control mailbox row。仍未完成的是把所有 mailbox 读写语义彻底提升为单一 AgentControl mailbox table，而不是当前“AgentControl control rows + session mailbox/event 兼容 mirror”的模型。
- child completion 已改为 parent AgentControl mailbox 主路径: child `session_end/session_interrupted` 先写 parent durable mailbox / `agent_control.subagent_completed` envelope，再追加 `display_mirror=true`、`mirror_source=agent_control_mailbox` 的兼容 `subagent.completed` session event；父 actor 不在线时仍能持久化 completion mailbox，且 display mirror 会记录 `mailbox_delivery_status`。普通 child/team mailbox、team task assignment 和 team terminal lifecycle mailbox 也已经通过共享 `internal/agentcontrol` envelope helper 收敛到 mailbox-first 投递。普通 child `send_message/followup_task`、child completion `subagent.completed`、team task assignment 和 team lifecycle 均携带统一 `message_type/control_action/workflow/mailbox_delivery/mailbox_kind` metadata，其中 child completion 的 envelope 构造和 display mirror 标记已进一步收口到 `toolbroker.BuildSubagentCompletionMailboxMessage` / `toolbroker.AnnotateSubagentCompletionDisplayMirror`，CLI/API 不再各自手写 metadata 拼装。当前 durable mailbox 已经接口化为可读可 watch 的 mailbox substrate，具备独立 session mailbox 表，并会把 AgentControl envelope 消息写入带独立 control sequence 的 `agent_control_mailbox_messages`，且可通过 AgentControl mailbox reader/watch 接口读取；但 AgentControl registry 与 mailbox read/watch 还未统一为单一控制面主路径。
- `spawn_team` 的 task assignment 已有 `TaskTriggerClient` seam，派发请求/结果已进入持久 `team_events` 审计流，并且目标 teammate session 也会收到 durable `agent_control.task_assignment` / `team.task_assignment` control mailbox message，可通过 mailbox substrate 读取；`task.completed/task.failed` 也会镜像为目标 teammate session 的 durable `agent_control.task_lifecycle` / `team.task_lifecycle` control mailbox message，blocked/handoff 通知也已补齐同类 task lifecycle envelope 并可被 `ListAgentControlMailbox` 读取。CLI 与 Runtime API 的 `list_agents` 也都已通过共享 AgentRegistry projection helper 把 teammate session 投影为 `/root/teams/<team>/<member>`，并能在 AgentStatus 中看到当前 `team_id/teammate_id/current_task_id/current_task_status`。新增的 `agentcontrol.TaskRecord` / `TaskFilter` / `TaskRegistryReader` / `TaskDependencyRecord` / `TaskDependencyFilter` / `TaskDependencyReader` / `TaskRegistryCreateWriter` / `TaskDependencyCreateWriter` / `TaskRegistryStatusWriter` / `TaskRegistryReleaseWriter` / `TaskRegistryLeaseRenewWriter` / `TaskRegistryClaimWriter` / `TaskRegistryTerminalWriter` / `TaskRegistryBlockWriter` 以及 `team.AgentControlTaskRegistry` adapter 已经把 team task 以统一 task read-model seam、dependency read seam、create writer seam、dependency writer seam、status writer seam、release writer seam、lease renew writer seam、claim writer seam、terminal writer seam 和 block writer seam 暴露给 AgentRegistry/debug/control-plane projection；Runtime API `CreateTask`、Runtime API `AddTaskDependency`、显式 AgentControl dependency endpoint、toolbroker `spawn_team` 初始 task/dependency 创建、`LeadPlanner` 自动 replan/follow-up task/dependency 创建、`ReleaseTaskLease` 与 `RenewTaskLease` 已使用对应 writer seam，`Orchestrator.ClaimReadyTasks` 已使用 claim writer seam，`ApplyTerminalTaskOutcome` 已使用 terminal writer seam，`ApplyBlockedTaskOutcome` 已使用 block writer seam；Runtime API 现在也有显式 `GET /api/runtime/agent-control/tasks`、`GET /api/runtime/agent-control/tasks/{task_id}/dependencies` 和 POST create/dependencies/status/claim/lease/release/terminal/block 写接口，typed client 已覆盖 `ListAgentControlTasks`、`ListAgentControlTaskDependencies` 与对应写方法，`skillsapi-demo` 也新增 `agent-control` 模式用于真实验证 task/dependency control-plane 入口。当前剩余缺口是 team task/dependency graph 底层仍不是单一 AgentControl 表权威，部分 workflow orchestration 仍使用 team-native API，还未全部迁入统一 AgentControl task graph/substrate。
- team orchestrator 已支持 mailbox sequence wake、task lifecycle sequence wake、lifecycle wake 和 fallback tick；mailbox wake 已经通过 `agentcontrol.MailboxWakeSource` seam 被 `Orchestrator.RunWithWake` 消费，`team.AgentControlMailboxWake` 作为当前 adapter 提供 durable sequence/watch；task lifecycle wake 也已经通过 `agentcontrol.TaskWakeSource` seam 被 `Orchestrator.RunWithWake` 消费，`team.AgentControlTaskRegistry` 作为当前 adapter 提供 durable sequence/watch。剩余缺口是两个 wake source 的底层权威仍分别是 team mailbox 和 team task signal 表，不是全局 AgentControl mailbox/task substrate watcher。
...
| child completion durable mailbox 通信模型 | child completion 已 mailbox-first: CLI/API 在收到 child terminal event 后先写 parent durable `agent_control.subagent_completed` mailbox，再追加 `display_mirror=true`、`mirror_source=agent_control_mailbox` 的兼容 `subagent.completed` session event；CLI/API 共用 `toolbroker.BuildSubagentCompletionMailboxMessage` 和 `toolbroker.AnnotateSubagentCompletionDisplayMirror`；公共 `DeliverMailboxEventFirst` 已验证优先走 AgentControl writer；Runtime API 已有 `GET /api/runtime/sessions/{id}/agent-control/mailbox`，typed client `ListSessionAgentControlMailbox` 与 `skillsapi-demo -agent-action control-mailbox` 可直接读取带独立 `control_seq` 的 control mailbox row | 接近完成但非最终形态 | child completion 语义已是 AgentControl mailbox 主路径；剩余缺口是底层仍保留 session mailbox/event mirror 用于兼容，还不是单一 AgentControl mailbox table/substrate 权威 |
| `spawn_team` task assignment 迁移到统一 AgentControl/substrate | `TaskTriggerClient` seam、dispatch requested/completed 事件、`team.task_assignment` / `team.task_lifecycle` durable mailbox、AgentRegistry teammate projection、AgentControl task read-model、dependency read-model、create writer seam、dependency writer seam、status writer seam、release writer seam、lease renew writer seam、claim writer seam、terminal writer seam 与 block writer seam 已完成并有测试；Runtime API `CreateTask`、Runtime API `AddTaskDependency`、显式 AgentControl dependency endpoint、toolbroker `spawn_team` 初始 task/dependency 创建、`LeadPlanner` replan/follow-up task/dependency 创建、`ReleaseTaskLease` / `RenewTaskLease` 已接入对应 writer，`Orchestrator.ClaimReadyTasks` 已通过 `ClaimAgentControlTask` 处理普通 claim 与 path-claim 原子分支，`ApplyTerminalTaskOutcome` 已通过 `UpdateAgentControlTaskTerminal` 处理 done/failed 终态落库，`ApplyBlockedTaskOutcome` 已通过 `BlockAgentControlTask` 处理 blocked/handoff 的 task blocked 落库；blocked/handoff mailbox 已补齐 `agent_control.task_lifecycle` envelope 并可通过 `ListAgentControlMailbox` 读取；Runtime API `GET /api/runtime/agent-control/tasks`、`GET /api/runtime/agent-control/tasks/{task_id}/dependencies` 和 POST create/dependencies/status/claim/lease/release/terminal/block 写接口与 typed client 已可直接读写统一 `TaskRecord` seam、dependency read seam 和 dependency writer seam；dependency edge read-model 已包含 edge `id` 与 `created_at`，dependency create 已进入 `task.dependency.created` team event 审计流；`skillsapi-demo -mode agent-control` 可验证 tasks/dependencies/add-dependency | 接近完成但非最终形态 | team task/dependency graph 仍不是单一 AgentControl 表权威；workflow sequence、全局 graph cursor 等仍未提升为统一 AgentControl task graph 字段；部分 workflow orchestration 仍由 team-native API 处理；派发、状态、依赖、终态和 blocked/handoff 通知虽然已有 seam/mirror，但还不是完整统一 AgentControl task graph/substrate |
| orchestrator wake 使用 durable mailbox watch/sequence | team mailbox sequence wake、task lifecycle signal sequence wake、lifecycle wake、fallback tick 已完成；mailbox wake 已抽象为 `agentcontrol.MailboxWakeSource` 并由 orchestrator 消费，task lifecycle wake 已抽象为 `agentcontrol.TaskWakeSource` 并由 orchestrator 消费；新增 `TestAgentControlMailboxWakeWatchesTeamMailbox`、`TestOrchestratorRunUsesAgentControlMailboxWakeSourceBeforeFallbackTick`、`TestAgentControlTaskRegistryWatchesTaskWake` 与 `TestOrchestratorRunUsesAgentControlTaskWakeSourceBeforeFallbackTick` 覆盖 | 接近完成但非最终形态 | mailbox/task wake 的默认 backing store 仍分别是 team mailbox 与 team task signal，不是单一全局 AgentControl mailbox/task substrate watcher |
