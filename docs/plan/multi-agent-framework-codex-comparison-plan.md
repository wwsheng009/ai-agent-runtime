...
- P2 child completion 父级 mailbox 镜像: child completion 现在还会向 parent session 写入一条 `mailbox_received` / `kind=subagent.completed` 的持久事件，metadata 透传 child session、parent、path、agent type、status、success/error 和原始 event seq；CLI `localActorRegistry` 与 runtime API `sessionAgentController` 均已覆盖。该路径现在优先通过 mailbox substrate 写入 durable mailbox row，再写兼容的 session event 并发布 runtime bus；父 actor 不在线或不可创建时也不会丢 completion 通知；actor delivery 只作为没有 durable mailbox/event store 时的兜底。
...
- P3 team task assignment control message 标准化: `team.task_assignment` mailbox envelope 现在携带 `message_type=agent_control.task_assignment`、`control_action=task.assign`、`workflow=spawn_team`、`mailbox_delivery=session_mailbox`、`mailbox_kind=team.task_assignment` 等标准 metadata。CLI `localActorRegistry.TriggerTask` 与 runtime API `sessionActorClient.TriggerTask` 的测试均证明 task assignment 不只存在于 `team_events` 审计流或 session event 镜像中，也可以通过 `MailboxReaderStore.ListMailbox` 从目标 teammate session 的 durable mailbox substrate 读取到。这把 `spawn_team` task assignment 从“裸 SubmitPrompt + 附属日志”推进为可替换 AgentControl substrate 能消费的 control/task message。
...
- P4 CLI 可见 agent/timeline/collab 入口: 新增 `/agents` 输出当前 agent tree，复用 `list_agents`/path 投影结果；当前已选默认目标会以 `selected=<target>` 行展示。新增 `/agents pick` / `/agents select`，在可用时复用现有 runtime selection popup，否则回退为编号输入选择器，可按编号、agent path、session id 或 agent id 选择并打印 agent 明细。新增 `/agents target <target|clear>`，可设置或清空默认 agent 消息目标；该选择写入 runtime session metadata context 的 `aicli_selected_agent_target`，恢复会话时会重新注入 `ChatSession.SelectedAgentTarget`。新增 `/agents send [target] <message>` 和 `/agents followup [target] <message>`，复用 `send_message/followup_task` 控制面显式向目标 child agent 投递 mailbox 或触发后续任务；省略 `target` 时使用 `/agents target` 或 picker 选中的默认目标。正常 root 用户输入仍不会被隐式重定向到 child，避免误把主对话 turn 发送给后台 agent。新增 `/timeline [limit]` 输出当前 active team 的持久 `team_events` 协作时间线，包括 seq、event type、task、session、assignee、status、via、success、trace、summary/error 等核心字段。`task.dispatch.requested/completed` 的派发请求、目标 session、AgentControl via 和执行结果现在可直接在 timeline 中审计。新增 `/collab [limit]` 输出当前 parent session mailbox/collab 时间线；当 store 支持 `MailboxReaderStore` 时，`/collab` 会优先读取 durable session mailbox substrate row，并标记 `source=mailbox`，同时过滤 session event mirror 避免重复展示，再补充 `subagent.completed/team.completed/team.summary` 等非 mailbox 协作终态事件；不支持 mailbox substrate 时才回退到兼容 session event 镜像。这便于直接排查 child completion、team summary 和 parent mailbox 通知是否 durable 到达。这些都是低风险 CLI/TUI 可见性入口；完整可切换 target 的多 agent 面板仍是后续 P4 工作。
...
- team mailbox 已具备 durable sequence/watch 基础，并且 team mailbox 到 session mailbox 的 dispatch 已是 mailbox-first；`wait_agent` 无目标调用与 runtime HTTP `/agents/wait` 空目标请求均已支持等待当前 parent session mailbox/collab event，`read_agent_events` 无目标调用与 runtime HTTP `/agents/events` 也已支持读取 parent mailbox/collab event，二者优先使用 mailbox substrate 并返回 mailbox `latest_seq` cursor，session event 只作为兼容 fallback。底层已有 `MailboxStore`/reader/watcher/sequence substrate 接口，runtime stores 也已原生实现 `AppendMailbox/ListMailbox/WatchMailbox/LastMailboxSeq`，SQLite 默认持久层已有独立 `session_mailbox_messages` 表；但它仍是 session-scoped mailbox，不是完整统一 AgentControl mailbox 表。
- child completion 已有 parent session 持久事件镜像和父 session `mailbox_received` durable 镜像，且不再依赖父 actor 在线；普通 child/team mailbox、team task assignment 和 team terminal lifecycle mailbox 也已经通过共享 helper/统一 envelope 收敛到 mailbox-first 投递，其中普通 child `send_message/followup_task` 已携带 AgentControl 风格 `message_type/control_action/workflow/mailbox_kind` metadata。当前 durable mailbox 已经接口化为可读可 watch 的 mailbox substrate 并具备独立 session mailbox 表，但尚未落到统一 AgentControl mailbox/registry 表。
- `spawn_team` 的 task assignment 已有 `TaskTriggerClient` seam，派发请求/结果已进入持久 `team_events` 审计流，并且目标 teammate session 也会收到 durable `agent_control.task_assignment` / `team.task_assignment` control mailbox message，可通过 mailbox substrate 读取；但还没有完全改为统一 AgentControl registry/substrate 派发。teammate task 仍由 team workflow 创建/claim，尚未成为统一 AgentControl task registry 中的 first-class task lifecycle 对象。
- team orchestrator 已支持 mailbox sequence wake、task lifecycle sequence wake、lifecycle wake 和 fallback tick；但这些仍是 team workflow 内部 watch，不是完整统一 AgentControl/substrate mailbox。
- 已有 `/agents`、`/agents pick`、`/agents target <target|clear>`、`/agents send [target] <message>`、`/agents followup [target] <message>`、`/timeline [limit]` 和 `/collab [limit]` 可见入口，timeline 已能审计 task dispatch 请求/完成与关键派发字段，collab 已优先查看 parent session mailbox substrate 并补充非 mailbox 协作终态事件，picker 已支持命令/弹窗选择并打印 agent 明细，默认 target 已能持久化到 runtime session context 并在恢复时还原，send/followup 已支持显式目标或默认目标两种方式向 child agent 投递 mailbox 或触发后续任务；但正常 root 输入不会隐式改投 child，完整可切换 target 的多 agent TUI 面板、以及富交互 collab timeline 仍是后续 P4 工作。
