...
| 13 | M5 | 🚧 进行中（slice 1-6 已落地：`apply` 执行核心 / export·import（API+CLI）/ TUI 生命周期子命令 / save-as 差分固化（TUI+API）；余：前端生命周期入口与 E2E-1~5 前端半程） | E2E-1~5 + A9-A12 | V22 已回填；V17 部分回填；V24、V25 待回填；Q19/Q20/Q21 |
