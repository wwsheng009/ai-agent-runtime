...
| U-2 / U-14 并发写与物理落点（§10.1，2026-09-22 增补） | ① 同一会话 override 的「读-改-写」在 TUI 与 API handler 两端各写各的：并发 PATCH 各自基于同一份旧快照落盘，后写者静默覆盖前者字段（M11 丢更新）；② workspace / config 两层同为「读-改-写」，TUI 与 API handler 共享同一对写入函数（`UpdateWorkspaceRoutingSection` / `UpdateAICLIRoutingSection`），两个会话绑定同一工作区也落到同一份 `chat-prefs.yaml`，同样没有跨写者串行化；③ U-14 的物理落点（会话 JSON / sqlite，且**不产生** `~/.aicli/workspaces`）此前无测试钉住 | ① 新增进程内会话写锁 `backend/internal/chat/session_write_lock.go`（`LockSessionWrite`：按会话 ID 注册、refs 计数 + 幂等释放、空 ID no-op）；API handler 的 session 分支**锁内重读**会话再合并（`reloadSessionForRoutingWrite`），TUI 的 `chatRoutingWriteKey` / `chatRoutingResetFromArgs` 取同一把锁（`chatRoutingWithSessionWriteLock` / `chatRoutingLockSessionWrite`），落盘即释放、runtime 重建不再占锁；② 新增路径级文件写锁 `backend/internal/agentconfig/config_file_write_lock.go`（`LockConfigFileWrite`：按目标文件路径注册、Windows 大小写归一），`UpdateWorkspaceRoutingSection` 与 `UpdateAICLIRoutingSection` 整段「读-改-写」入锁；同日第二次增补把同一份配置文件的**其余写点**（chat / theme / provider / provider 管理 / proxy / 层文档 / workspace prefs / starter 配置）全部并入同一把锁与共享写事务 `config_file_write.go`（`updateConfigFileDocument`），锁范围由「仅 routing」扩展为「本包全部配置写点」（§12「并发写串行化范围」已同步更新）；同日第三次增补把 runtime-server 对同一份 config 的三个写点（`agent.maxSteps` 落盘 / skills_runtime 策略落盘 / `SaveDocument` 文档保存）也接入同一把锁，并新增多文件取锁助手 `LockConfigFileWriteAll`（按归一化锁键去重 + 字典序取锁，防自锁与 ABBA） | `session_routing_handlers_test.go`：`TestSessionRoutingConcurrentWritesDoNotLoseFields`（U-2：`racingRoutingStorage` 把丢更新窗口变成确定性顺序，两字段都必须保留）、`TestSessionRoutingSessionLayerPhysicalLanding`（U-14：文件后端 `sessions/YYYY/MM/DD/<id>.json` 的 `metadata.context.aicli_routing_override` + sqlite 读回 + `~/.aicli/workspaces` 不存在）；`config_file_write_lock_test.go`：`TestLockConfigFileWriteBlocksUntilReleased`、`TestConfigFileWritersHonorSharedWriteLock`（9 个写点逐一占锁 → 每个写入都必须等待）、`TestWorkspacePrefsWritersHonorSharedWriteLock`、`TestConcurrentConfigSectionWritersPreserveEverySection`（4 写者并发 ×8 轮，各节最后一次写入都在）、`TestUpdateWorkspaceRoutingSectionSerializesConcurrentWriters`、`TestUpdateAICLIRoutingSectionSerializesConcurrentWriters`。routing 两个并发用例 + 本轮统一用例均经「临时摘锁 → 必须 FAIL → 还原复跑 ok」的证伪验证（摘 `UpdateAICLIChatPreferences` 的锁 → `TestConfigFileWritersHonorSharedWriteLock/chat` FAIL）；`go test -race ./internal/agentconfig/` 亦通过；**第三次增补（包外写者）**：`backend/internal/runtimeserver/config_write_lock_test.go` 的 `TestRuntimeConfigWritersHonorSharedWriteLock`（agent_max_steps / skills_runtime_policy / config_document_save 三个包外写点：先占住写锁 → 每个写入都必须等待（200ms 内不得完成）→ 释放后必须成功；临时摘掉这三个写点的锁 → 三个子用例全 FAIL → 还原复跑 ok，还原后三个文件 SHA256 与摘锁前逐一一致）+ `config_file_write_lock_test.go` 的 `TestLockConfigFileWriteAllDedupesRepeatedTargets`（同一路径的三种写法只取一次锁，未去重即自锁超时）、`TestLockConfigFileWriteAllHoldsEveryTarget`（事务里每个目标都被占住；统一释放后探测者逐个取回并归还）；**第四次增补（分层保存的锁编排）**：`runtimeserver/config_write_lock_layered_test.go` 的 `TestLayeredConfigDocumentSaveWaitsOnTargetLayerLock`（占住**目标层文件**的锁 → 分层保存必须等待；能直接完成即层写入绕过锁）、`TestLayeredConfigDocumentSaveDoesNotSelfLock`（改**项目层所有**的键时目标层文件就是 documentPath：若分层分支不先释放本文档锁，按层取锁即自锁、保存永不返回，用例用 10s 超时钉成失败）、`config_file_write_lock_test.go` 的 `TestLockConfigFileWriteAllOppositeOrdersDoNotDeadlock`（相反顺序请求同一对文件必须都能完成 + 临界区互斥；去掉字典序即 ABBA 死锁）。三条新用例均经「临时破坏 → 必须 FAIL → 还原复跑 ok」证伪（去掉字典序 → 死锁超时；摘掉层文件锁 → 保存越过锁直接完成；分层分支不释放本文档锁 → 自锁超时），三个生产文件还原后 SHA256 与破坏前逐一一致；**第五次增补（静态守卫，防御性加强）**：`backend/internal/configwriteguard/guard_test.go` 的 `TestConfigFileWritesStayUnderSharedWriteLock`——源码级 AST 守卫（config 语义路径的 `os.WriteFile` / `ioutil.WriteFile` 与全部 `writeFileAtomic` / `writeFilePreserveMode` 调用必须处于共享写锁之下；判据含同目录调用图向上 6 层；带扫描规模自检与豁免过期检查；实测 1333 文件 / 11 受保护写点 / 1 豁免）。证伪：新增一条裸写探针 → FAIL 并精确指出 `文件:行:函数`；抽掉 `PersistRuntimeAgentMaxSteps` 的锁 → 其 3 个写点立即判为未受保护而 FAIL；两次破坏均已还原（生产文件 SHA256 与破坏前一致，探针文件已删） |
...
| R4 | 并发写丢更新 | §3.4 串行化 + U-2；进程内按会话/路径串行化 + 跨进程旁路文件 OS 锁（§9.1 增补；降级退回后写胜出）+ 外部改动由 `ConfigExternalReloader` 兜底刷新；后写胜出并留 warning；**新增裸写通道**由源码级静态守卫挡在测试闸门（`backend/internal/configwriteguard::TestConfigFileWritesStayUnderSharedWriteLock`：非测试源码里 config 语义路径的 `os.WriteFile` / `ioutil.WriteFile` 与全部 `writeFileAtomic` / `writeFilePreserveMode` 调用都必须处于共享写锁之下，否则用例失败） |
...
- **2026-09-22 增补（防御性加强：配置写锁静态守卫）**：把「配置写点必须走共享写锁」从约定升级为**源码级守卫用例** `backend/internal/configwriteguard`（独立零依赖包，只依赖标准库、只做 AST 分析、不 import 被测代码，因此不受仓库其它包编译状态影响）。判据：`backend/` 下全部非测试 Go 文件中，① 目标表达式含 config/cfg 语义的 `os.WriteFile` / `ioutil.WriteFile`，② 任何 `writeFileAtomic` / `writeFilePreserveMode` 调用（不论目标变量名），都必须处于共享写锁之下——「受保护」判据为：函数体内出现 `LockConfigFileWrite` / `LockConfigFileWriteAll`，或沿**同目录**调用图向上 ≤6 层能到达这样的函数，或在 `guardWriteExemptions` 显式登记理由（当前仅 `EnsureUserPresetsFile` 一项，create-once 独立文件）。守卫自带两重自检（扫描文件数 <800、受保护写点 <8 即判「判据失效」直接失败）与豁免过期检查（豁免未命中 → 报错要求删除）；`scripts/` 下一次性 AICLI_HOME 的 e2e 驱动整目录排除（非产品内共享配置的并发写者），`testdata` / `vendor` / `node_modules` / `.git` 同样跳过；守卫的已知局限（不做数据流分析，改名后的裸写、`os.Create`+`io.Copy` 写法判不出来）已在用例注释中如实登记。实测：扫 1333 个非测试 Go 文件、11 个受保护写点、1 个豁免。证伪演练：① 在 `internal/runtimeserver` 放一条裸写探针 → 用例 FAIL 并精确指出 `文件:行:函数`；② 抽掉 `PersistRuntimeAgentMaxSteps` 的写锁 → 其 3 个写点立即判为未受保护而 FAIL；两次破坏均已还原（生产文件 SHA256 与破坏前一致、探针文件已删除、复跑 PASS）。
