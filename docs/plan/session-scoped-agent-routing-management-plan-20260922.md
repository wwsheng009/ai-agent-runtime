...
- P2 已知边界（诚实记录）：`thinking_effort`/`max_tokens`/`temperature` 无目录候选，面板第三级给「直接写入」提示而不伪造建议值（§5.3 的数值输入项留待后续增量）；`availability`/`candidates` 为只读字段，不进面板字段列表。
...
- **面板「只读打开」的落地形态（如实登记）**：§5.6/I-8 的原文是「子会话中的 `/routing` 面板只读打开」。当前实现取**入口退化**形态：子会话执行 `/routing`（含 `main`/`sub` 直达）时返回只读投影摘要 + 说明行，而不是进入「可导航但不可写」的全屏选择器——避免出现「能走到第三级、按下才被拒」的假入口。若后续要求真正的只读全屏导航，需给 `chatPickerStage` 增加只读模式，属独立增量（已在此登记，不作为本方案交付前提）。
...
- **快照刷新边界（如实登记）**：进程内快照的刷新只覆盖「经本 handler 的 PATCH 写入」这一条路径。外部进程或手工编辑配置文件、以及不经该 handler 的其它写入路径**不会**刷新快照——需重启 runtime-server 或等待下一次配置重载（`SetAICLIConfig` 是 runtime-server 的唯一注入点）。会话级覆盖随会话记录持久化（`Session.Metadata.Context`），不受此边界影响；workspace 层由每次投影/解析时读文件，也不受此边界影响。
...
- **并发写串行化范围（U-2，如实登记）**：写入锁是**进程内**的——会话锁按会话 ID、文件锁按目标文件路径注册，因此 aicli TUI 与内嵌 runtime-server 的 API handler、以及两个绑定同一工作区的会话，命中的都是同一把锁。跨进程并发（另开一个 aicli 进程、手工编辑文件、外部写入）不在锁范围内：落盘的原子写（`writeFileAtomic` / FileStorage 写锁 / SQLite 事务）只保证单次写入不撕裂，不保证不丢更新（该场景为**后写胜出**，与 §3.4 的冲突语义一致）。workspace 层锁键为目标 `chat-prefs.yaml` 路径（Windows 下大小写归一），config 层锁键为按层路由后的配置文件路径（项目层与全局层各自成键）。
...
| R4 | 并发写丢更新 | §3.4 串行化 + U-2；后写胜出并留 warning |
