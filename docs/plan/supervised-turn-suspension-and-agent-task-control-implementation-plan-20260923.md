...
> **状态**：**P0-前置 / P0 / P1 / P2 / P3 已全部实施并验收闭环**（逐项证据见 §13 实施注记；P3 门禁收口总览在 §13.14 末尾）。剩余仅 **L4 上线后事项**：生产实例基线采样、上线后度量对比、AC-P3-4b 的真实 ≥2h 墙钟。与设计稿冲突时**以设计稿为准**；本文件新增的分期归属（#13/#15/#16 的落位）与验收编号（`AC-*`）为施工约定。
>
> **修订记录**：
...
> 逐项落地状态与验证证据见 **§13 实施注记**（P0–P3 已全部闭环；P3 门禁收口总览在 §13.14 末尾）。本清单保留原始条目，不逐条回勾，避免与 §13 的证据记录产生第二份事实源。
...
**P3 门禁收口总览（§13.9–§13.13 条目里的 ⏳ 以本表为准）**

| 门禁项 | 收口条目 | 状态 |
| --- | --- | --- |
| C4-1 阻塞分支移除 + `wait` 取值保留映射 | §13.9 | ✅ |
| C4-2 保留窗口 / GC / 驱逐后重建 / takeover | §13.10 | ✅ |
| C4-3 重启恢复与围栏 | §13.11 | ✅ |
| C4-4 端到端验收 | §13.12（AC-P3-4a 由 §13.14 在 C0-E 提交后复验） | ✅ L2 形态；AC-P3-4b 的「真实 ≥2h 墙钟」与 AC-P3-4c 的「相对基线改善」为 **L4 留白**（上线后按同一读数对比） |
| EC-F 组（3 条场景） | §13.12 | ✅ |
| EC-G 组（4 条场景） | §13.13（EC-G4 补齐） | ✅ |
| C0-E 度量基线采集 | §13.14 | ✅ 首次采样（本机 dev/CLI 库）；生产基线待部署实例一次采样 |
| §7.3 DoD「度量基线 + 上线后对比记录」 | §13.14 | 前半（基线）✅；后半（上线后对比）待发布节奏 |
