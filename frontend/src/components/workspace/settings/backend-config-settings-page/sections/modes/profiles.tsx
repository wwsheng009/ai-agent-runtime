...
import { useProfileLifecycleActions } from "./use-profile-lifecycle-actions";
...
export function ProfilesModeSection() {
  ...
  // 生命周期动作（rename / duplicate / move / delete）在独立 hook 内（P0-2 行数门禁）：
  // 状态仍由本组件持有，hook 只做动作与回执，语义与拆分前逐字一致。
  const { runRename, runDuplicate, runMove, runDelete } = useProfileLifecycleActions({
    t,
    setEntries,
    setView,
    setDialog,
