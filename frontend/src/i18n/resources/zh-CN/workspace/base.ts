...
export const zhWorkspaceBase = {
  ...
  composer: {
    ...
    attachments: {
      ...
      uploaded: "已上传",
      uploadFailed: "上传失败",
      uploadingCount: "{{count}} 个附件上传中",
      readyCount: "{{count}} 个附件已上传，将随消息发送",
      unsettledBlocked: "{{count}} 个附件未上传完成，暂不能发送",
      blockedUnsettled: "有 {{count}} 个附件未上传完成：重试或移除后再发送",
      needsSession: "新会话需先发送一条文字消息创建会话，之后才能发送图片附件",
      sent: "已发送 {{count}} 张图片",
      sentWithNotes: "已发送 {{count}} 张图片；服务端说明：{{notes}}",
      busyPending: "会话正忙，本轮未执行：附件仍在草稿轨，可重试发送",
      sendFailed: "带图发送失败：{{reason}}",
      sendUnavailable: "带图发送接口不可用：{{reason}}",
      rejected: "已忽略 {{count}} 个文件（重复、超大或超出数量上限）",
      dropInvitation: "松开以添加附件",
      error: {
        unavailable: "附件上传接口不可用",
        validation: "上传请求被拒绝",
        rejected: "服务端未接受该图片",
        failed: "上传失败",
      },
