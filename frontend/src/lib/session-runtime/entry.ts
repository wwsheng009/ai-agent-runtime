...
function countPending(state: PendingInteractionState): SessionRuntimePendingCounts {
  let approvals = 0;
  let questions = 0;
...
export function createSessionRuntimeEntry(
  ...
  function startLoop(mode: SubscriptionMode) {
    ...
    if (mode === "live") {
      void runLiveLoop(token, controller);
    } else {
      void runPollLoop(token, controller);
    }
  }
